...
function setStatus(text, close, context)
	...
	if close == "blacklist" then
		...
		refs.status.Text =
			"Your aztup hub license has been revoked for violating our terms of services. Continuous violations of our terms of services, could result in you not being allowed to purchase Aztup Hub. The reason of this decision can be found below. If you think this is a mistake please do not hesitate to contact the support. If you think this decision was fair you can re-purchase a license on the website dashboard by clicking on re-purchase."
	...
	elseif close == "hwid" then
		...
		refs.status.Text = string.format(
			"This hwid has changed from the one we have in our records, to continue to use the script and confirm that you are owning the script, please input the 6 digits code that we've just sent to you to your email %s (make sure to check your spams).",
			context.email
		)
		...
		refs.button.MouseButton1Click:Connect(function()
			...
			if req.Success then
				...
				pcall(function()
					errorMessage = HttpService:JSONDecode(req.Body).message
				end)
...
xpcall(function()
	...
	local suc, err = pcall(function()
		if not isfolder("Aztup Hub V3") then
			makefolder("Aztup Hub V3")
		end
		if not isfolder("Aztup Hub V3/scripts") then
			makefolder("Aztup Hub V3/scripts")
		end
	end)
	...
	if not suc then
		logError(err)
		setStatus(
			"Failed to create scripts folder, this could be caused by executor not being located in a proper directory.",
			true
		)
		return
	end
	...
	repeat
		task.wait()
	until universeId and metadataRequest
	...
	if isfile("Aztup Hub V3/scripts/loader.lua") then
		...
		if not loaderHash then
			loaderHash = hash(readfile("Aztup Hub V3/scripts/loader.lua"))
		end
	...
	if scriptOutdated then
		...
		repeat
			task.wait()
		until base_append and require_loader
