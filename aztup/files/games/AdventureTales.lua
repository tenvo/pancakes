...
local function getWeaponInfo()
	...
	local weaponInfo = {}
	if wep1 then
		table.insert(weaponInfo, ItemData:FindFirstChild(wep1.ItemID.Value, true))
	end
	if wep2 then
		table.insert(weaponInfo, ItemData:FindFirstChild(wep2.ItemID.Value, true))
	end
...
local function getAttackSpeed()
	...
	if weapon1 and FindFirstChild(wepInfo1, "AttackSpeed") then
		attackSpeed = (
			1
			/ (wepInfo1.AttackSpeed.Value * 1.03 ^ (weapon1.Rarity.Value - wepInfo1.BaseRarity.Value) * (1 + 0 / 100))
		)
	end
	...
	if weapon2 and FindFirstChild(wepInfo2, "AttackSpeed") then
		local tempSpeed = (
			1
			/ (wepInfo2.AttackSpeed.Value * 1.03 ^ (weapon2.Rarity.Value - wepInfo2.BaseRarity.Value) * (1 + 0 / 100))
		)
		if attackSpeed > tempSpeed then
...
do --functions
	...
	function functions.killAura(toggle)
		...
		if getWeaponType()[1] ~= "Melee" and getWeaponType()[2] ~= "Melee" then
			ToastNotif.new({ text = "Only Melee weapons can use this feature", duration = 10 })
		...
		maid.killAura = RunService.Heartbeat:Connect(function()
			...
			for _, npc in next, npcs do
				if
					(npc.rootPart.Position - rootPartPos).Magnitude > library.flags.killAuraDistance
					or npc.humanoid.Health <= 0
				then
					continue
				end
				table.insert(t, {
			...
			if #t <= 0 or tick() - didAt < attackSpeed then
				return
			end
			didAt = tick()
	...
	function functions.attackRate(toggle)
		...
		if getWeaponType()[1] ~= "Melee" and getWeaponType()[2] ~= "Melee" then
			ToastNotif.new({ text = "Only Melee weapons can use this feature", duration = 10 })
			library.options.attackRate:SetState(false)
		end
	end
	...
	function functions.autoSell(toggle)
		...
		maid.autoSell = Utility.listenToChildAdded(Stats.Inventory, function(obj)
			if not obj:FindFirstChild("Rarity") then
				return
			end
		...
		maid.autoSellLoop = RunService.Heartbeat:Connect(function()
			if tick() - lastAutoSell <= 1 or #itemsToSell == 0 then
				return
			end --Don't fire remote unnecessarily
