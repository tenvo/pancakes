...
local ReplicatedFirst, Players, RunService, Lighting, ReplicatedStorage, UserInputService =
	Services:Get("ReplicatedFirst", "Players", "RunService", "Lighting", "ReplicatedStorage", "UserInputService")
...
do -- // Scanning
	...
	if not frameworkLoad then
		return LocalPlayer:Kick("[AR2] Error 1, dm Aztup.")
	end
...
do -- // Hooks
	...
	local function onNetworkFetch(self, remoteName, ...)
		...
		if
			remoteName ~= "Get Server Debug State"
			and remoteName ~= "Ping Return"
			and remoteName ~= "Set Character State"
			and remoteName ~= "Ping"
			and debugMode
		then
	...
	function bullets:Fire(a, b, weapon, d, direction)
		...
		if
			weapon.Attachments
			and weapon.Attachments.Ammo
			and weapon.Attachments.Ammo.Amount == 0
			and library.flags.autoReload
		then
...
do -- // Utilities
	...
	function setupLootEsp()
		local function onLootGroupStateChange(behaviorHeritage, part, instance)
			if instance:GetAttribute("LootGroupState") == "Fresh" then
				lootEspItems[part] = lootEspBase.new(
					part,
					require(ReplicatedStorage.Chunking["Container Data"]:FindFirstChild(behaviorHeritage)).DisplayName,
					nil,
					true
				)
	...
	function killAura(toggle)
		...
		maid.killAura = RunService.Heartbeat:Connect(function()
			...
			for i, v in next, characters do
				...
				if not rootPart or v == LocalPlayer.Character then
					continue
				end
	...
	function autoHeal(toggle)
		...
		maid.autoHealLoop = RunService.Heartbeat:Connect(function()
			if tick() - lastUpdatedAt < 1 then
				return
			end
			lastUpdatedAt = tick()
			local player = players:get()
			local inventory = player.Character and player.Character.Inventory
			if not inventory or not player.Character or player.Character.Health:Get() > 80 then
				return
			end

			for i, v in next, inventory.Containers[1].Occupants do
				if v.Type == "Medical" and v.UseValue.Health then
					local animLength = resources
						:Find("ReplicatedStorage.Assets.Animations." .. v.ConsumeConfig.Animation)
						:GetAttribute("Length")
	...
	function autoOpenDoor(toggle)
		...
		maid.autoOpenDoor = RunService.Heartbeat:Connect(function()
			local myRootPart = LocalPlayer.Character and LocalPlayer.Character.PrimaryPart
			if not myRootPart or tick() - lastRan <= 0.5 then
				return
			end
			...
			for i, v in next, Interactables do
				if
					v.Type == "Door"
					and v.State == "Closed"
					and (myRootPart.Position - v.Instance.PrimaryPart.Position).Magnitude <= 25
				then
					v:Interact()
				end
			end
	...
	function bringLoots(toggle)
		...
		maid.bringLoots = RunService.Heartbeat:Connect(function()
			...
			if not inventory then
				return print("[Bring Loot] Inventory not found")
			end
			...
			for chunkName in next, nearChunks do
				...
				for _, group in next, chunk:GetChildren() do
					...
					if not position or (position - myPosition).Magnitude > 20 then
						continue
					end
					...
					for _, container in next, inventory.Containers do
						if container.DisplayName == "Pockets" or container.IsCarried then
							continue
						end

						for _, occupant in next, container.Occupants do
							if not filteredTypes[occupant.Type] then
								continue
							end
...
Main:AddToggle({ text = "Bring Loots", callback = bringLoots })
	:AddList({ flag = "Bring Loots Values", values = lootTypes, multiselect = true })
