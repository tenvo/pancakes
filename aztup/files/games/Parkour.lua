...
do -- // Utility
	...
	local function onCharacterAdded()
		...
		repeat
			wait()
		until mainEnv.updateGrapplerPos
	...
	function blackBubbleChat(t)
		ReplicatedStorage.BubbleColours[LocalPlayer.Name].Value = t and Color3.fromRGB(30, 30, 30)
			or Color3.fromRGB(255, 255, 25)
	end
	...
	do -- // Render Distance Setup
		local chunkObjectsList = {
			workspace.Props,
			workspace.TimeTrials,
			workspace.Billboards,
			workspace.BustableDoors,
			workspace.MapDecoration,
		}
		local chunkObjects
		...
		local function updateChunk(disable)
			...
			if not rootPart or not chunkObjects then
				return
			end
...
parkourMisc
	:AddToggle({ text = "Enable Render Distance", callback = enableRenderDistance })
	:AddSlider({ flag = "Render Distance Value", min = 250, max = 5000 })
