...
local ReplicatedStorage, Players, MemStorageService, TeleportService, HttpService =
	Services:Get("ReplicatedStorage", "Players", "MemStorageService", "TeleportService", "HttpService")
...
if table.find(GAME_PLACE_IDS, game.PlaceId) then
	...
	dataLoss:AddButton({
		text = "Undo old Dataloss",
		callback = function()
			funcs.setUpDataloss(false)
		end,
	})
	...
	function funcs.setUpDataloss(t)
		if
			t
			and not library:ShowConfirm(
				"Please note none of your slot will load for 30 mins once you'll start clicking on the spin btn this will get fixed after 30mins"
			)
		then
			return
		end
...
if game.PlaceId == LOBBY_PLACE_ID then
	...
	function funcs.rollBackData(t, bypass)
		...
		for _, v in next, myData.Customization:GetChildren() do
			originalFunctions.fireServer(
				customizationEvent,
				{ ["Number"] = n, ["Type"] = v.Name, ["Action"] = "Update" }
			)
			originalFunctions.fireServer(
				customizationEvent,
				{ ["Hsv"] = Color3.new(n, n, n), ["Type"] = "SetColor", ["Action"] = "Update", ["Act"] = v.Name }
			)
			originalFunctions.fireServer(
				customizationEvent,
				{ ["Hsv"] = Color3.new(n, n, n), ["Type"] = "SetColor2", ["Action"] = "Update", ["Act"] = v.Name }
			)
		end
	...
	if MemStorageService:HasItem("doSpinSlot") then
		library.OnLoad:Connect(function()
			if not ReplicatedStorage.Player_Datas:FindFirstChild(localPlayer.Name) then
				...
				repeat
					task.wait()
				until ReplicatedStorage.Player_Datas:FindFirstChild(localPlayer.Name)
					or tick() - gameLoadedAt > (debugMode and 30 or 60)
