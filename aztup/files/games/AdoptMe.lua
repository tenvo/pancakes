...
local ReplicatedStorage, Players, RunService, UserInputService, MemStorageService, TeleportService, NetworkClient, GuiService =
	Services:Get(
		"ReplicatedStorage",
		"Players",
		"RunService",
...
Autofarm:AddToggle({
	text = "No Rendering",
	callback = function(t)
		RunService:Set3dRenderingEnabled(not t)
	end,
})
...
local Actions = {
	...
	dirty = function(data, pet)
		...
		task.spawn(function()
			remotes["HousingAPI/ActivateFurniture"]:InvokeServer(
				LocalPlayer,
				getFurniture("generic_shower"),
				"UseBlock",
				{
	...
	bored = function(data)
		...
		local BoredAilmentTarget =
			workspace:WaitForChild("StaticMap"):WaitForChild("Park"):WaitForChild("BoredAilmentTarget")
		local ranAt = tick()
	...
	hot_spring = function(data)
		...
		local hotSpringCF =
			workspace:WaitForChild("StaticMap"):WaitForChild("HotSpring"):WaitForChild("HotSpringOrigin")
		local ranAt = tick()
...
coroutine.wrap(function()
	while task.wait(1) do
		...
		if not ailmentsCompleted then
			print("no ailment completed")
			continue
		end

		if
			not petCharWrapper
			or not petCharWrapper.ailments_monitor
			or not petCharWrapper.ailments_monitor.ailments
		then
			print("no pet char wrapper")
			continue
		end
		...
		if willHatch and autoDataLoss then
			...
			task.spawn(function()
				...
				if library.flags.wantedPets[pet.name] then
					Webhook.new(library.flags.webhookUrl)
						:Send(string.format("@everyone You got a %s %s", pet.rarity, pet.name), true)

					-- disdplay webhook notif;
					funcs.unsetDataloss()
				else
					Webhook.new(library.flags.webhookUrl)
						:Send(string.format("You got a unwanted %s %s", pet.rarity, pet.name), true)
				end
