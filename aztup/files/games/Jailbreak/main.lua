...
local Players, ReplicatedStorage, RunService, CollectionService, UserInputService =
	Services:Get("Players", "ReplicatedStorage", "RunService", "CollectionService", "UserInputService")
...
function Utility:isTeamMate(player)
	...
	local myTeam = (LocalPlayer.Team.Name == "Criminal" or LocalPlayer.Team.Name == "Prisoner") and "Criminal"
		or "Police"
...
do -- // Functions
	...
	do -- // Get Network & Other Stuff
		...
		function playerUtils.isPointInTag(pointPosition, tag)
			if tag == "NoFallDamage" and library.flags.antiFallDamage then
				return true
			end
			if tag == "NoRagdoll" and library.flags.antiRagdoll then
				return true
			end
		...
		function militaryTurretUtils.getNearestNonPolice(...)
			...
			if player == LocalPlayer and library.flags.disableTurrets then
				return
			end
		...
		function cargoShipTurret:Shoot()
			if library.flags.disableTurrets then
				return
			end
	...
	function functions.speedHack(toggle)
		...
		maid.speedHack = RunService.Stepped:Connect(function()
			...
			bodyVelocity.Velocity =
				camera.CFrame:VectorToWorldSpace(ControlModule:GetMoveVector() * library.flags.speedValue)
	...
	function functions.fly(toggle)
		...
		maid.fly = RunService.Stepped:Connect(function()
			...
			bodyVelocity.Velocity =
				camera.CFrame:VectorToWorldSpace(ControlModule:GetMoveVector() * library.flags.flyValue)
	...
	function functions.noWait(toggle)
		...
		maid.noWait = RunService.Heartbeat:Connect(function()
			if not circleActionModule.Spec then
				return
			end
	...
	function functions.clickDestroy(toggle)
		...
		maid.clickDestroy = UserInputService.InputBegan:Connect(function(input, gpe)
			if input.UserInputType.Name ~= "MouseButton1" or gpe then
				return
			end
			if
				not playerMouse.Target
				or not playerMouse.Target:IsA("BasePart")
				or playerMouse.Target:IsA("Terrain")
			then
				return
			end
