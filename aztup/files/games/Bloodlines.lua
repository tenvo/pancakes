...
-- Services
local Players, ReplicatedStorage, RunService, Lighting, MemStorageService, TeleportService =
	Services:Get("Players", "ReplicatedStorage", "RunService", "Lighting", "MemStorageService", "TeleportService")
...
do
	...
	do
		...
		function funcs.instantLog()
			if inDanger then
				return ToastNotif.new({ text = "You can not do this right now. You are in danger." })
			end
	...
	do
		...
		function funcs.teleportToPlayer()
			...
			if not player or not player.rootPart then
				return
			end
		...
		function funcs.findThunderstormServer(state)
			...
			if not oldServerList or #oldServerList == 0 then
				...
				while true do
					...
					if not serverList.Success then
						continue
					end
					...
					if not serverList.nextPageCursor or not serverList.data then
						break
					end
	...
	function funcs.flyHack(state)
		...
		maid.flyStepped = RunService.Stepped:Connect(function()
			...
			maid.flyBodyVelocity = maid.flyBodyVelocity and maid.flyBodyVelocity.Parent ~= nil and maid.flyBodyVelocity
	...
	function funcs.rollbackData()
		...
		if
			library:ShowConfirm(
				inDanger
						and "You are in danger. Are you sure you want do this right now? Rollback data does not work 100% of the time"
					or "Are you sure you want to do to this"
			)
		then
			doRollback()
		end
	...
	function funcs.removeFF()
		local character = localPlayerData.character
		if not character or not character:FindFirstChildWhichIsA("ForceField") then
			return
		end
