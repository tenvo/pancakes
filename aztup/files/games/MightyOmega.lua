...
local ReplicatedStorage, Players, RunService, Lighting, UserInputService, VirtualInputManager, TeleportService, PathfindingService, MarketPlaceService, guiService =
	Services:Get(
		"ReplicatedStorage",
		"Players",
		"RunService",
...
local function checkIfMod(v)
	...
	if not library.flags.modNotifier or not role or not modroles[role] then
		return
	end
	if library.flags.autoPanic then
		library:Unload()
	end
...
local function parseKey(str)
	return Utility.find({ str:byte(1, 9999) }, function(v)
		return v > 128
	end)
end
...
local function getKey(script)
	...
	for _, v in next, env do
		...
		for _, k in next, getupvalues(v) do
			if typeof(k) ~= "string" or not parseKey(k) then
				continue
			end
	...
	for _, v in next, script.Parent:GetDescendants() do
		...
		if not con or not con.Function then
			continue
		end
...
local function safeClick(part) --Part should have a vector3 and contain ClickDetector
	if not library.flags.useMouseClick then
...
repeat
	task.wait()
until ffc(plr, "Backpack")
repeat
	task.wait()
until ffc(plr.Backpack, "LocalS")
--Inits
...
repeat
	task.wait()
until ffcwia(plr.Backpack, "Tool")
if execmenu then
	task.wait(2)
end
...
local function initGC()
	for _, v in next, getgc() do
		if
			typeof(v) == "function"
			and islclosure(v)
			and not is_synapse_function(v)
			and table.find(getconstants(v), "F1ySuspicion")
		then
...
oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
	...
	if library.flags.infRhythm then
		...
		local args = { ... }
		if args[2] == "RhythmStance" and args[3] == false then
			return
		end
	end
...
local function isBusy()
	if library.flags.autoEat and Stats.Hunger <= library.flags["autoEatAt%"] then
		repeat
			task.wait()
		until Stats.Hunger >= library.flags["eatTo%"]
			or (
				not library.flags.legitAutoMachine
				and not library.flags.riskyAutoMachine
				and not library.flags.autoDura
			)
	end
	if library.flags.autoProtein and getFood("Protein Shake") then
		repeat
			task.wait()
		until Stats.ProteinShake
			or (
				not library.flags.legitAutoMachine
				and not library.flags.riskyAutoMachine
				and not library.flags.autoDura
...
local function getBag()
	...
	for i, v in next, punchingBags do
		if
			((v.Position * Vector3.new(1, 0, 1)) - (char.HumanoidRootPart.Position * Vector3.new(1, 0, 1))).magnitude
			< 6.5
		then
			closest = v
			break
...
local function getPlayerInRange(ignoredCharacter, targetPos, range)
	...
	for i, v in next, LivingThings:GetChildren() do
		if v == ignoredCharacter then
			continue
		end
		if not (ffc(v, "HumanoidRootPart")) then
			continue
		end
		if (v.HumanoidRootPart.Position - targetPos).magnitude >= range then
			continue
		end
		inRange = v
		break
...
local function getMobInRange(range)
	...
	for i, v in next, LivingThings:GetChildren() do
		...
		if ffc(Players, v.Name) then
			continue
		end
		if not (ffc(v, "HumanoidRootPart")) or not ffc(char, "HumanoidRootPart") then
			continue
		end
...
function autoDura.new(firstTurn, secondTurn)
	...
	self._maid:GiveTask(
		self._firstChar.Humanoid.HealthChanged:Connect(function(health) --When the first turn gets too low
			if (health / self._firstChar.Humanoid.MaxHealth * 100) >= library.flags["minimumHp%"] then
				return
			end
			if self._firstDebounce then
				return
			end
			...
			repeat
				task.wait()
			until self._firstChar.Humanoid.Health >= self._firstChar.Humanoid.MaxHealth or not self._firstChar.Parent

			if self._lastTurn ~= char then
	...
	self._maid:GiveTask(
		self._secondChar.Humanoid.HealthChanged:Connect(function(health) --When the second turn gets too low
			if (health / self._secondChar.Humanoid.MaxHealth * 100) >= library.flags["minimumHp%"] then
				return
			end
			if self._secondDebounce then
				return
			end
			...
			repeat
				task.wait()
			until self._secondChar.Humanoid.Health >= self._secondChar.Humanoid.MaxHealth or not self._secondChar.Parent
	...
	self._maid:GiveTask(RunService.RenderStepped:Connect(function()
		if tick() - self._lastRefresh <= 0.1 then
			return
		end
		self._lastRefresh = tick()

		if self._stopPunching then
			return
		end
		if not self._duraVal or not self._duraVal.Parent then
			return
		end
		if not self._lastTurn or self._lastTurn == char then
			return
		end --Only start punching when they have actually popped dura
		if self._combatStyle.Parent ~= char then
			char.Humanoid:UnequipTools()
			self._combatStyle.Parent = char
			return
		end
...
function autoDura:Unpop()
	...
	repeat
		task.wait()
	until not getToolByName("Durability Training")

	self:BuyDura()
	if not library.flags.takeTurns then
		return
	end --If take turns is not enabled then don't start punching
...
function autoDura:BuyDura()
	if not self._duraButton then
		self:CreateError("Stand closer to a durability training button")
		return
	end
	if getToolByName("Durability Training") then
		return
	end
...
function autoDura:Pop()
	if self._lastTurn and self._lastTurn == char and library.flags.takeTurns then
		return
	end --If it was our turn last and take turns is on then don't pop
	if self._lastTurn and self._lastTurn ~= char and not library.flags.takeTurns then
		return
	end --If it wasn't our turn last and take turns isn't on then don't pop
	if not self._duraTool or not self._duraTool.Parent then
		self:BuyDura()
	end --Try to buy the tool

	if not self._duraTool or not self._duraTool.Parent then
		return
	end --If still no tool then return
	...
	repeat
		task.wait(0.1)
	until char.Humanoid.Health >= char.Humanoid.MaxHealth --Wait till full health to pop
	repeat
		task.wait(0.1)
	until not Stats.isEating
...
do --Entity esp overwrite
	...
	local function onCharacterAdded(player) --Normally this should be a character but we pass a player..
		if not player.Character:WaitForChild("CurrentStamina", 10) then
			return
		end
	...
	local function onNewMobAdded(mob, espConstructor)
		if ffc(Players, mob.Name) then
			return
		end
		if not mob:WaitForChild("HumanoidRootPart", 5) then
			return
		end
		...
		connection = mob.AncestryChanged:Connect(function()
			if mob:IsDescendantOf(game) then
				return
			end
...
local a = function()
	...
	local function spectatefunc(Object)
		...
		if not ffc(LivingThings, Object.Name) then
			return
		end --If they dont have a player model
	...
	plrGUI.ChildAdded:Connect(function(c)
		...
		if not c:WaitForChild("Frame", 2) then
			return
		end
		if not c.Frame:WaitForChild("ScrollF", 2) then
			return
		end
		repeat
			task.wait()
		until ffc(c.Frame.ScrollF, plr.Name)
		task.wait(0.1)
	...
	Misc:AddToggle({
		text = "Auto Leave",
		tip = "Sometimes risky as they chase people who insta log",
		callback = function()
			return
		end,
	})
	...
	Misc:AddToggle({
		...
		function(toggle)
			...
			repeat
				...
				if closestPlayer then
					plr:Kick("Player got too close to you, auto kicked.")
				end
	...
	Misc:AddButton({
		text = "Rejoin Server",
		callback = function()
			TeleportService:TeleportToPlaceInstance(4878988249, game.JobId, plr)
		end,
	})
	--Safe Functions
	...
	Safe:AddToggle({
		...
		callback = function(toggle)
			...
			gMaid.autoTrainMaid = RunService.Stepped:Connect(function()
				...
				if eating or Stats.Sleeping or Stats.isKnocked or Stats.isEating or tryingToSleep then
					sprinting = false
					return
				end
				...
				if trainMove == "Stamina" then
					if Stats.Stamina < staminaP and not Stats.isEating then
						sprinting = false
						env.stopSprint()
						repeat
							task.wait()
						until (Stats.Stamina + 1 >= library.flags["maxStamina%"]) or not library.flags.autoTrain
					end --Waiting for stamina
					if not Stats.isEating and not sprinting and not Stats.Sleeping and not Stats.isKnocked then
						sprinting = true
						env.runPrompt()
					end --If they can run then make them run

					autoTrainDeb = false
					return --If training stamina then return
				end

				if not tool or tool.Name ~= trainMove then
					tool = ffc(plr.Backpack, trainMove) or ffc(char, trainMove)
				end --If they changed what training they are using then switch the tool
				if not tool or not tool.Parent then
					library.options.autoTrain:SetState(false)
					autoTrainDeb = false
					return
				end --If no tool or tool is in nil then turn off and return
				if
					tool.Parent ~= char
					and not Stats.isEating
					and not Stats.isKnocked
					and not Stats.Sleeping
					and not eating
				then
					char.Humanoid:EquipTool(tool)
					task.wait(0.2)
				end --If they arent doing something and dont have the tool equipped, equip the tool
				if eating or Stats.Sleeping or Stats.isKnocked then
					autoTrainDeb = false
					return
				end --If they are busy then return
				if Stats.Stamina < staminaP then
					repeat
						task.wait()
					until (Stats.Stamina + 1 >= library.flags["maxStamina%"]) or not library.flags.autoTrain
				end --If has less than staminaP task.wait till max stamina

				if library.flags.trainingType == "Slow" then
					repeat
						task.wait()
					until not Stats.isSquatting or not library.flags.autoTrain
					repeat
						task.wait()
					until not Stats.isPushuping or not library.flags.autoTrain
				end

				if tool.Parent ~= char then
					autoTrainDeb = false
					return print("Couldn't equip tool")
				end
				tool:Activate()
				...
				if library.flags.trainingType == "Slow" then
					...
					task1 = task.spawn(function()
						if library.flags.training == "Push up" then
							repeat
								task.wait()
							until Stats.isPushuping or not library.flags.autoTrain
						elseif library.flags.training == "Squat" then
							repeat
								task.wait()
							until Stats.isSquatting or not library.flags.autoTrain
						end
						task.cancel(task2)
						autoTrainDeb = false
					end)
	...
	Safe:AddSlider({
		text = "Stamina %",
		tip = "When the training action will pause until max stamina %",
		float = 0.1,
		min = 0,
		max = 100,
		callback = function(val)
	...
	Safe:AddSlider({
		text = "Max Stamina %",
		tip = "When the training action will resume",
		value = 99,
		min = 0,
		max = 100,
		float = 0.1,
	...
	Safe:AddToggle({
		...
		callback = function(toggle)
			...
			strikeMaid:GiveTask(plrGUI.ChildRemoved:Connect(function(v)
				...
				if ffc(plr.Backpack, "Strike Speed Training") or ffc(char, "Strike Speed Training") then
					return
				end
				...
				if
					((button.Head.Position * Vector3.new(1, 0, 1)) - (char.HumanoidRootPart.Position * Vector3.new(
							1,
							0,
							1
						))).magnitude
						> 10
					and library.flags.autoWalk
				then
				...
				char.Humanoid:UnequipTools()
				local strikeTool = plr.Backpack:WaitForChild("Strike Speed Training", 5)
					or ffc(char, "Strike Speed Training")

				if not strikeTool then
					return
				end
			...
			if ffc(plrGUI, "SpeedTraining") then
				...
				char.Humanoid:UnequipTools()
				if
					library.flags.autoWalk
					and (
							(button.Head.Position * Vector3.new(1, 0, 1))
	...
	Safe:AddToggle({
		text = "Auto Walk",
		tip = "This function is very obvious and clippable",
		callback = function()
			return
		end,
	})
	...
	autoPunch:AddToggle({
		text = "Auto Punch",
		tip = "This is a Striking Power Macro",
		callback = function(toggle)
			if not toggle then
				gMaid.autoPunchMaid = nil
				gMaid.tryM2 = nil
				return
			end
			...
			gMaid.autoPunchMaid = RunService.Stepped:Connect(function()
				if autoPunchDeb then
					return
				end
				if eating or Stats.isKnocked or not fightTool then
					return
				end
				...
				if fightTool.Parent ~= char then
					char.Humanoid:UnequipTools()
					fightTool.Parent = char
				end --If the fightTool parent isnt char then give it to char
				if eating then
					repeat
						task.wait()
					until (not eating) or not library.flags.autoPunch
				end --If eating then task.wait till not
				...
				if (Stats.Stamina + 1 >= 80) or Stats.Stamina <= library.flags["minStamina%"] then
					...
					repeat
						task.wait()
					until Stats.Stamina >= library.flags["staminaWait%"] or not library.flags.autoPunch
				end
	...
	autoPunch:AddSlider({
		text = "Min Stamina %",
		tip = "Will run until this % to sweat",
		min = 0,
		max = 100,
		float = 0.1,
		callback = function()
	...
	autoPunch:AddSlider({
		text = "Stamina Wait %",
		tip = "Will wait till this % to punch",
		value = 99,
		min = 0,
		max = 100,
		callback = function()
		...
		callback = function(t)
			...
			local function canAttack()
				return UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)
					and plr.Character
					and plr.Character:FindFirstChildWhichIsA("Tool")
			end
	...
	legitAutoMachine:AddToggle({
		text = "LEGIT Auto Machine",
		callback = function(toggle)
			if not toggle then
				legitMachineMaid:DoCleaning()
				doingAction = false
				return
			end
			doingAction = false
			...
			legitMachineMaid:GiveTask(plrGUI.ChildAdded:Connect(function(v)
				task.wait()
				if ffc(v, "Machine") then
					lastMachine = v.Machine.Value
				end

				if v.Name == "BarbellMachineGUI" or v.Name == "SquatMachineGUI" then
					if not v:WaitForChild("Frame2") then
						return
					end
					if not v.Frame2:WaitForChild("LiftingF") then
						return
					end
					doingAction = true
					...
					legitMachineMaid:GiveTask(v.Frame2.LiftingF.ChildAdded:Connect(function(z)
						...
						if Stats.Stamina <= library.flags["minimumStamina%"] then
							...
							repeat
								task.wait()
							until Stats.Stamina >= library.flags["maximumStamina%"]
								or not library.flags.legitAutoMachine
							stupidWait = false
						end --Wait for stamina to reach 100%
					...
					if not library.flags.autoReuse then
						return
					end
					if not v:WaitForChild("Frame") then
						return
					end
					if not v.Frame:WaitForChild("ListF") then
						return
					end

					local powerButton =
						v.Frame.ListF:WaitForChild(string.format("Barbell %s Weight", library.flags["lift/squatPower"]))
					...
					if not library.flags.legitAutoMachine then
						return
					end

					repeat
						task.wait()
					until (Stats.Stamina >= 100) or not library.flags.legitAutoMachine

					repeat
						safeButton(v.Frame2.Start)
						task.wait()
					until not v.Frame2.Start.Visible
						or (not library.flags.legitAutoMachine or not library.flags.autoReuse)
					return
				end

				--Treadmill Part
				if v.Name ~= "TreadmillMachineGUI" then
					return
				end
				if not v:WaitForChild("Frame3") then
					return
				end
				if not v.Frame3:WaitForChild("TrainingF") then
					return
				end

				doingAction = true
				task.wait()
				legitMachineMaid:GiveTask(
					v.Frame3.TrainingF.ButtonTemplate:GetPropertyChangedSignal("Position"):Connect(function()
						if stupidWait then
							return
						end
						if Stats.Stamina <= library.flags["minimumStamina%"] then
				...
				if not ffc(v.Frame, library.flags.treadmillType, true) then
					return
				end
				...
				repeat
					...
					task.wait()
				until not v.Parent
					or not v.Frame.Visible
					or (not library.flags.legitAutoMachine or not library.flags.autoReuse)

				repeat
					task.wait()
				until v.Frame2.ListF:FindFirstChild(library.flags.treadmillPower) or not library.flags.autoReuse

				repeat
					safeButton(ffc(v.Frame2.ListF, library.flags.treadmillPower, true))
					task.wait()
				until not v.Parent
					or not v.Frame2.Visible
					or (not library.flags.legitAutoMachine or not library.flags.autoReuse)

				repeat
					task.wait()
				until v.Frame3.Visible or not library.flags.legitAutoMachine
			...
			legitMachineMaid:GiveTask(plrGUI.ChildRemoved:Connect(function(v)
				if
					v.Name ~= "BarbellMachineGUI"
					and v.Name ~= "SquatMachineGUI"
					and v.Name ~= "TreadmillMachineGUI"
				then
					return
				end
				...
				repeat
					...
					task.wait()
				until ffc(plrGUI, "BarbellMachineGUI")
					or ffc(plrGUI, "SquatMachineGUI")
					or ffc(plrGUI, "TreadmillMachineGUI")
					or not library.flags.autoReuse
					or not library.flags.legitAutoMachine
			end))
		end,
	})

	legitAutoMachine:AddSlider({
	...
	legitAutoMachine:AddSlider({
		text = "Reuse Wait",
		tip = "How long to wait before getting on the machine",
		value = 2,
		min = 0,
		max = 2,
		float = 0.1,
	...
	autoDuraSettings:AddToggle({
		...
		callback = function(state)
			...
			if not player1 or not player2 then
				autoDura:CreateError("Cannot find one or both of the players.")
				return
			end
			if not player1.Character or not player2.Character then
				autoDura:CreateError("One of the player's isn't spawned in")
				return
			end
	...
	autoDuraSettings:AddBox({
		text = "First Turn",
		tip = "This value should be the same for both users",
		value = "First Player",
	})
	autoDuraSettings:AddBox({
		text = "Second Turn",
	...
	local function eat(item, ignore, keepEating)
		if getgenv().debug_auto_eat then
			print(
				...
				Stats.Sleeping,
				tryingToSleep,
				library.flags.autoEat
			)
		end
		if typeof(item) == "table" then
			item = unpack(item)
		end --If we pass a table to it handle it properly
		if Stats.Hunger >= library.flags["autoEatAt%"] and not ignore then
			return
		end
		if eating then
			return
		end
		if Stats.isEating then
			return
		end
		...
		if Stats.isKnocked then
			return false
		end
		if Stats.Sleeping then
			return false
		end
		if tryingToSleep then
			return false
		end
		if not item then
			return false
		end
		if not library.flags.autoEat then
			return false
		end
		warn("PASSED ALL")
		...
		if doingAction then
			char.Humanoid:UnequipTools()
			return false
		end --We want to make sure we NEVER have food on treadmill
		...
		until Stats.isEating or not library.flags.autoEat --If item is in nil or they are on the treadmill then break, otherwise equip
		repeat
			task.wait(0.5)
		until not Stats.isEating or not library.flags.autoEat
		...
		callback = function(toggle)
			...
			gMaid.AutoSleep = RunService.Stepped:Connect(function()
				...
				elseif Stats.Fatigue <= library.flags["wakeAt%"] and Stats.Sleeping then --Wake up
					if library.flags.autoEat and getFood() == nil then
						return
					end
					toggleSleep()
				end
	...
	local autoRhythm
	autoRhythm = Safe:AddToggle({
		text = "Auto Rhythm",
		callback = function(toggle)
			if not toggle then
				gMaid.autoRhythm = nil
				return
			end
			...
			gMaid.autoRhythm = RunService.Stepped:Connect(function()
				if not (tick() - lastRhythmLoop >= 0.2) then
					return
				end

				lastRhythmLoop = tick()

				if char.HumanoidRootPart.RhythmUI.Enabled then
					return
				end
				if fightTool and fightTool.Parent ~= char then
					return
				end
				if Stats.Rhythm >= 100 then
					return
				end
				if Stats.isEating then
					return
	...
	Safe:AddToggle({
		...
		callback = function(toggle)
			if not toggle then
				gMaid.autoFlowToggle = nil
				return
			end
			if not ffc(plrGUI, "FlowUI") then
				library.options.autoUseFlow:SetState(false)
				return
			end

			gMaid.autoFlowToggle = plrGUI.FlowUI:GetPropertyChangedSignal("Enabled"):Connect(function()
				if not plrGUI.FlowUI.Enabled then
					return
				end
	...
	local function autoSprint(toggle)
		...
		gMaid.autoSprint = UserInputService.InputBegan:Connect(function(input, gpe)
			if gpe or tick() - lastRan < 0.1 then
				return
			end
	...
	function networkOneShot.new(mob)
		...
		self._maid:GiveTask(RunService.RenderStepped:Connect(function()
			if library.flags.oneShotPercent < self.humanoid.Health / self.humanoid.MaxHealth * 100 then
				return
			end
			self.char:PivotTo(
				CFrame.new(self.hrp.Position.X, workspace.FallenPartsDestroyHeight - 500, self.hrp.Position.Z)
			)
		end))
	end
	...
	risky:AddButton({
		...
		callback = function()
			...
			ToastNotif.new({
				text = "Buy money service 1m for 15$ at https://shoppy.gg/product/mBzd1gV , it has also been copied to your clipboard",
			})
		end,
	})
	...
	risky
		:AddToggle({
			text = "One Shot NPC",
			tip = "Do not kill enemies too quickly as you will get logged",
			callback = function(toggle)
				if not toggle then
					gMaid.OwnerShipBeat = nil
					gMaid.OneShotNPC = nil
					networkOneShot:ClearAll()
					return
				end
				...
				gMaid.OneShotNPC = Utility.listenToChildAdded(LivingThings, function(instance)
					if ffc(Players, instance.Name) then
						return
					end
					if not instance:WaitForChild("HumanoidRootPart", 5) or not instance:WaitForChild("Humanoid", 5) then
						return
					end
	...
	risky:AddToggle({
		...
		callback = function(toggle)
			...
			gMaid.attachToback = RunService.Heartbeat:Connect(function()
				...
				if not target or not ffc(target, "HumanoidRootPart") then
					return
				end

				char.HumanoidRootPart.CFrame = target.HumanoidRootPart.CFrame
					* (CFrame.new(0, library.flags.attachToBackDistance, 1) * angleOffSet)
	...
	risky:AddSlider({
		...
		tip = "How far to move you from the mob",
		value = 5,
		min = 1,
		max = 10,
		float = 0.1,
	...
	risky:AddSlider({
		...
		tip = "Get closest mob in this range",
		value = 100,
		min = 0,
		max = 100,
		float = 1,
	...
	do --Risky AutoMachine
		...
		function riskyMachine:Setup()
			self._maid:GiveTask(Utility.listenToChildAdded(plrGUI, function(machineUI)
				if
					machineUI.Name ~= "TreadmillMachineGUI"
					and machineUI.Name ~= "BarbellMachineGUI"
					and machineUI.Name ~= "SquatMachineGUI"
				then
					return
				end
			...
			self._maid:GiveTask(plrGUI.ChildRemoved:Connect(function(machineUI)
				if self._lastMachineName ~= machineUI.Name then
					return
				end
				...
				if not library.flags.riskyAutoMachine then
					return
				end
				...
				repeat
					...
					task.wait()
				until ffc(plrGUI, self._lastMachineName)
					or not library.flags.autoReuse
					or not library.flags.riskyAutoMachine
			end))
		end
		...
		function riskyMachine:Treadmill()
			...
			until not v.Parent
				or not v.Frame.Visible
				or (not library.flags.riskyAutoMachine or not library.flags.autoReuse)
			...
			repeat
				task.wait()
			until v.Frame3.Visible or not library.flags.riskyAutoMachine
			...
			repeat
				task.wait()
			until (Stats.Stamina >= 100) or not library.flags.riskyAutoMachine
		...
		riskyAutoMachine:AddToggle({
			text = "Risky Auto Machine",
			tip = "Makes gaining all stats faster through machines, uses settings from legit auto machine",
			callback = function(t)
				if not t then
					return riskyMachine:ClearAll()
				end
				...
				spawn(function()
					repeat
						...
						elseif (CONTROL.L + CONTROL.R) == 0 and (CONTROL.F + CONTROL.B) == 0 and SPEED ~= 0 then
							BV.velocity = (
								(camera.CoordinateFrame.lookVector * (lCONTROL.F + lCONTROL.B))
								+ (
									(
										camera.CoordinateFrame
	...
	local function teleport(target)
		...
		if typeof(cframe) == "Vector3" or typeof(cframe) == "CFrame" then
			...
			teleMaid:GiveTask(plr.CharacterAdded:Connect(function()
				teleportFunc(cframe)
			end))
			char.Humanoid.Health = 0
		elseif typeof(target) == "Instance" and target:IsA("Part") then
			teleMaid:GiveTask(plr.CharacterAdded:Connect(function()
				teleportFunc(cframe.CFrame)
			end))
			char.Humanoid.Health = 0
		else
		end
	...
	risky:AddToggle({
		text = "Inf Rhythm",
		tip = "Makes your rhythm inf, makes a cloud under your feet",
		callback = function(toggle)
			if not toggle then
				return
			end
			...
			local key = getKey(plr.Backpack.LocalS)
			if not key then
				library.options.infRhythm:SetState(false)
				return
			end
	...
	task.spawn(function()
		while task.wait(1) do
			...
			if not ffc(char, "MaxStamina") then
				return
			end
	...
	local function autoParry(v)
		...
		autoBlockMaid:GiveTask(animator.AnimationPlayed:Connect(function(animationTrack)
			...
			if
				not distance
				or distance > library.flags.autoParryRange
				or not parryAnims[id]
				or not (math.random(1, 100) <= library.flags.autoParryChance)
			then
				return
			end

			if willGuardbreak or parryAnims[id]["Guardbreak"] then
				if ffc(char, "Blocking") then
				...
				if library.flags.autoCounter then
					guardBreak()
	...
	autoParrySettings:AddToggle({
		text = "Auto Parry",
		callback = function(toggle)
			if not toggle then
				autoBlockMaid:DoCleaning()
				return
			end
	...
	local function notifyWebhook(message)
		if library.flags.webhookUrl == "" then
			return error("Webhook URL must not be empty")
		end
		syn.request({
	...
	Notify:AddToggle({
		...
		callback = function(toggle)
			...
			gMaid.autoKick = RunService.Stepped:Connect(function()
				if
					(library.flags.hunger and library.flags["hungerReaches%"] >= Stats.Hunger)
					or (library.flags.fatigue and library.flags["fatigueReaches%"] <= Stats.Fatigue)
				then
	...
	Notify:AddToggle({
		...
		callback = function(toggle)
			...
			gMaid.deadKick = char.Humanoid.Died:Connect(function()
				if not library.flags.autoKick then
					return
				end
	...
	do --Street Fighter ESP/Notifier
		local function onStreetFighterAdded(streetFighter)
			if not streetFighter:WaitForChild("HumanoidRootPart", 10) then
				return
			end
			local streetFighterName = streetFighter:WaitForChild("Attached", 1)
					and streetFighter.Attached:WaitForChild("FakeH", 1)
					and streetFighter.Attached.FakeH:FindFirstChildWhichIsA("Model")
					and streetFighter.Attached.FakeH:FindFirstChildWhichIsA("Model").Name
				or "Street Fighter"
			...
			connection = streetFighter.AncestryChanged:Connect(function()
				if streetFighter:IsDescendantOf(game) then
					return
				end
		...
		local function sfDescendantAdded(instance)
			if not instance or instance.Name ~= "NPCModel" then
				return
			end
			...
			if not library.flags.streetFighterNotifier then
				return
			end

			ToastNotif.new({ text = "A Street Fighter has spawned: " .. streetFighterName })
			if library.flags.webhookNotify then
				notifyWebhook("@everyone A Street Fighter has spawned: " .. streetFighterName)
			end
		end
	...
	do --Behelit ESP/Notifier
		...
		local function bhDescendantAdded(instance)
			if not instance or instance.Name ~= "BehelitMODEL" then
				return
			end

			onBehelitAdded(instance)

			if not library.flags.behelitNotifier then
				return
			end

			ToastNotif.new({ text = "A Behelit has spawned!!!" })
			if library.flags.webhookNotify then
				notifyWebhook("@everyone A Behelit Necklace has spawned")
			end
		end

		Notify:AddToggle({
			text = "Behelit Notifier",
			callback = function(toggle)
				if not toggle then
					return
				end
