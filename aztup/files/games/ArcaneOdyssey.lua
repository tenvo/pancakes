...
local ReplicatedStorage, Players, RunService, CollectionService, Lighting, UserInputService, VirtualInputManager, TeleportService, MemStorageService, TweenService, HttpService, Stats, NetworkClient, GuiService =
	Services:Get(
		"ReplicatedStorage",
		"Players",
		"RunService",
...
do --Misc Functions
	...
	local function getColdWater()
		for i, v in next, workspace.Map.Whitesummit:GetDescendants() do
			if not v:IsA("StringValue") or v.Value ~= "Freshwater" then
				continue
			end
	...
	function functions.revealMap()
		for i, v in next, map:GetChildren() do
			if not v:FindFirstChild("DetailsLoaded") then
				continue
			end
	...
	function functions.autoEat(t)
		...
		updateHunger.OnClientEvent:Connect(function(hungerValue)
		...
		maid.autoEat = RunService.Stepped:Connect(function()
			...
			last = tick()
			if currentHunger >= library.flags.autoEatValue then
				return
			end
	...
	function functions.autoFish(t)
		if not t then
			maid.autoFishCast = nil
			fishClock:FireServer("StopClock")
			return
		end
	...
	function functions.noUnderwaterEffect(t)
		...
		local seaDarkness = LocalPlayer:FindFirstChild("PlayerGui")
			and LocalPlayer.PlayerGui:FindFirstChild("SeaDarkness")
		if seaDarkness then
	...
	function functions.noFog(t)
		if not t then
			Lighting.Atmosphere.Density = lastFogDensity
			maid.noFog = nil
			return
		end
...
do --Local Cheat Functions
	...
	function functions.hitBoxExpander(t)
		if not t then
			attackClient.AttackSizeMulti = oldAttackSizeMulti
			return
		end
	...
	function functions.hideName()
		...
		if not head or not head:FindFirstChild("Overhead") then
			return
		end
	...
	function functions.speedHack(toggle)
		...
		maid.speedHack = RunService.Heartbeat:Connect(function()
			...
			maid.speedHackBv.Velocity = (
				humanoid.MoveDirection.Magnitude ~= 0 and humanoid.MoveDirection
				or gethiddenproperty(humanoid, "WalkDirection")
			) * library.flags.speedHackValue
		end)
	end
	...
	function functions.infiniteJump(toggle)
		...
		repeat
			...
			if rootPart and UserInputService:IsKeyDown(Enum.KeyCode.Space) then
				rootPart.Velocity =
					Vector3.new(rootPart.Velocity.X, library.flags.infiniteJumpHeight, rootPart.Velocity.Z)
			end
			task.wait(0.1)
...
do -- Player ESP
	...
	function EntityESP:Plugin()
		...
		return {
			text = string.format(
				"\n[Level: %s] [Awakening: %s]",
				playerStats.level or "None",
				playerStats.awakening or "None"
			),
		}
	end
...
do -- ESP
	...
	function Utility:renderOverload(data)
		...
		makeNpcForFolders(
			{ RS.UnloadEnemies, workspace.Enemies },
			enemiesESP,
			{ espName = "Enemies", showHealthFlag = "Enemies Show Health" }
		)
...
localCheats
	:AddToggle({ ----DealWeaponDamage,DealStrengthDamage hook: If you are the 2nd damage argument then fire the remote multiple times
		text = "Hit Multiplier",
...
misc:AddButton({
	text = "Teleport To Story Marker",
	tip = "Teleports you to your current story objective if you have one",
	callback = functions.teleportToStoryObjective,
})
misc:AddButton({
	text = "Teleport To Quest Marker",
	tip = "Teleports you to your current quest objective if you have one",
	callback = functions.teleportToQuestObjective,
})
...
misc:AddToggle({
	...
	tip = "Enabling this will also increase your fps",
