...
do -- Render gui
	...
	do -- // Aimbot Whitelist
		...
		local function addPlayer(userId, isTextBox)
			...
			if not userId then
				return print("no user id", userId, isTextBox)
			end
		...
		aimbotWhitelist:AddList({
			text = "Players",
			flag = "Aimbot Whitelist Players",
			playerOnly = true,
			noSave = true,
		})
		...
		aimbotWhitelist:AddButton({
			text = "Add Player By User Id",
			callback = function()
				addPlayer(tonumber(library.flags.aimbotWhitelistPlayerBox), true)
			end,
		})
	end
end
