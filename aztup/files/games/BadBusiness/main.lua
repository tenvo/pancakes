...
do -- // Functions
	...
	for _, v in next, getupvalues(TS) do
		if
			typeof(v) == "table"
			and getrawmetatable(v)
			and typeof(rawget(getrawmetatable(v), "__index")) == "function"
		then
	...
	function makeUnlockSkins(name)
		local folder = name == "Charms" and getupvalue(gameModules.Charms.GetConfig, 1)
	...
	do -- // ESP Override
		...
		function Utility:getCharacter(player)
			...
			if not characterBody then
				return
			end

			local health = character:FindFirstChild("Health")
			if not health or (player ~= LocalPlayer and health:FindFirstChild("Shield")) then
				return
			end
	...
	do -- // Utility
		...
		local function getBulletDirectionForSilentAim()
			...
			if not bulletStats or not bulletStats.Speed then
				return
			end
			...
			local bulletDropPrediction = library.flags.bulletDropPrediction
					and Vector3.new(0, 0.5 * bulletStats.Gravity * timeToShoot ^ 2, 0)
				or Vector3.new()

			local playerMovePrediction = library.flags.playerMovePrediction
					and (Utility:roundVector(targetHead.Velocity) * timeToShoot)
				or Vector3.new()
