...
local Players, ReplicatedStorage, RunService, TweenService =
	Services:Get("Players", "ReplicatedStorage", "RunService", "TweenService")
	...
	do -- grab framework
		...
		if not framework then
			return LocalPlayer:Kick(
				"Error Occured -> A. Dm Aztup. Do not dm if you are in the lobby just do not execute the script in the lobby"
			)
		end
	end
		...
		if respectFlags then
			if library.flags.targetMobs then
				for _, mob in next, workspace.Mobs:GetChildren() do
					if
						mob.PrimaryPart
						and (mob.PrimaryPart.Position - myRootPart.Position).Magnitude <= closestDistance
						and mob:FindFirstChild("Entity")
						and mob.Entity.Health.Value > 0
					then
			...
			if library.flags.targetPlayers then
				for _, player in next, game.Players:GetChildren() do
					if
						...
						and (player.Character.PrimaryPart.Position - myRootPart.Position).Magnitude
							<= closestDistance
					then
	...
	function toggleAutoFarm(toggle)
		while library.flags.toggleAutoFarm do
			...
			if target and myRootPart then
				...
				TweenService:Create(
					myRootPart,
					TweenInfo.new(targetDistance / 50),
					{ CFrame = target.PrimaryPart.CFrame + Vector3.new(0, 5, 0) }
				):Play()
			end
