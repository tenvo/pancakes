...
local Players, RunService, UserInputService, ReplicatedFirst =
	Services:Get("Players", "RunService", "UserInputService", "ReplicatedFirst")
...
function Utility:getCharacter(player)
	...
	if not playerCharacter or not playerCharacter.isAlive then
		return
	end
...
local function getAimPart()
	...
	if currentGun.weaponType == "Melee" then
		return
	end
	return body
end
...
killFeedContainer.ChildAdded:Connect(function(child)
	if not library.flags.killSay then
		return
	end
	task.wait()
...
RunService.RenderStepped:connect(function(step)
	...
	if (weaponColor or weaponMaterial) and camera:FindFirstChild("Main") then
		for _, v in next, camera.Main:GetChildren() do
			if weaponColor then
				v.Color = weaponColorValue
			end
			if weaponMaterial then
				v.Material = weaponMaterialValue
			end
...
do
	...
	localSection
		...
		:AddSlider({
			...
			max = 100,
			callback = function(value)
				if not library.flags.walkspeed then
					return commEvent:Fire(7, 16)
				end
