...
local ReplicatedStorage, RunService, Players, VirtualInputManager, CollectionService, MarketplaceService, Stats =
	Services:Get(
		"ReplicatedStorage",
		"RunService",
		"Players",
...
do
	...
	do
		...
		function fireServerWrapper(self, name, ...)
			...
			elseif name == "MeleeDamage" then
				...
				if typeof(args[2]) == "Instance" and library.flags.antiParry then
					...
					if shield and shield.Enabled then
						return print("no dmg?")
					end
				end
			...
			elseif name == "RangedFire" and library.flags.magicBullet then
				...
				table.foreach(args[3], function(i)
					shotId = i
				end)
		...
		oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
			...
			if
				method == "FireServer"
				and IsA(self, "RemoteEvent")
				and self.Name == "GotHitRE"
				and library.flags.noSelfDamage
			then
				return
			end
		...
		oldIndex = hookmetamethod(game, "__index", function(self, p)
			...
			if
				p == "FloorMaterial"
				and library.flags.noFallDamage
				and IsA(self, "Humanoid")
				and debug.info(isSynapseV3 and 2 or 3, "s"):find("FallDamageHandlerClient")
			then
				return plasic
			end
		...
		function unequipToolsWrapper(...)
			if library.flags.antiUnequipTools then
				return
			end
		...
		oldCancelReload = hookfunction(RangedWeaponClient.cancelReload, function(...)
			if library.flags.noReloadCancel then
				return {}
			end
		...
		local function getMouseHitPositionWrapper(...)
			if not debug.info(3, "s"):find("RangedWeaponClient") then
		...
		oldCalculateFireDirection = hookfunction(RangedWeaponHandler.calculateFireDirection, function(a, ...)
			if not library.flags.noSpread then
				return oldCalculateFireDirection(a, ...)
			end
			return a.LookVector
		end)
	...
	do
		...
		Utility.listenToChildAdded(workspace.PlayerCharacters, function(char)
			...
			humanoid.AnimationPlayed:Connect(function(animationTrack)
				if not animations[animationTrack.Animation.AnimationId:match("%d+")] or not library.flags.autoParry then
					return
				end
				...
				if
					...
					> rightArm.Size.Magnitude + hitbox.Size.Magnitude + 1.5
				then
					return
				end
	...
	function functions.instaUse(t)
		for _, utilityToolMetaData in next, UtilityMetadata do
			if not utilityToolMetaData.useTime then
				continue
			end
	...
	function functions.fastRespawn(t)
		...
		maid.fastRespawnOne = RunService.Heartbeat:Connect(function()
			if tick() - lastRanAt < 0.1 then
				return
			end
			if RoduxStore.store:getState().health.current > 0 then
				return
			end
			lastRanAt = tick()
		...
		maid.fastRespawnTwo = RunService.Heartbeat:Connect(function()
			if tick() - lastRanAt2 < 0.1 then
				return
			end
			if not RoduxStore.store:getState().mainMenuClient.isIn then
				return
			end
			lastRanAt2 = tick()
