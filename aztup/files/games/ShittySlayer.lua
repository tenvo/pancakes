...
local ReplicatedStorage, Players, MemStorageService, TeleportService =
	Services:Get("ReplicatedStorage", "Players", "MemStorageService", "TeleportService")
...
if game.PlaceId == GAME_PLACE_ID or game.PlaceId == GAME_PLACE_ID2 then
	...
	function functions.setUpDataloss()
		...
		if #mapLocations:GetChildren() >= 21 then
			return ToastNotif.new({ text = "You can already use dataloss go join the lobby game!" })
		end
		if yen <= 25200 then
			return ToastNotif.new({ text = "You do not have enough yen for this feature you need atleast 25200" })
		end --You don't have enough yen for this feature

		--Disable notifications
		getrenv()._G.Notify = function()
			return
		end
		...
		repeat
			task.wait()
		until mapUI.Map_Points.Value >= 21 --Wait for us to have more than 21 map points
		...
		repeat
			task.wait(0.1)
		until #mapLocations:GetChildren() >= 21
...
if game.PlaceId == LOBBY_PLACE_ID then
	...
	function functions.rollBackData(t)
		...
		if #unlockedLocations:GetChildren() < 21 then
			ToastNotif.new({ text = "You cannot use this feature without using setup dataloss in the main game!" })
			return false
		end --You can't rollback your data currently please join the main game and try!
		...
		for _, v in next, unlockedLocations:GetChildren() do
			task.spawn(function()
				...
				if t then
					repeat
						task.wait()
					until t and v.Value == longStr or not t and v.Value == v.Name
				end
			...
			while true do
				...
				if suc then
					...
					for clanDisplayName, value in next, library.flags.clans do
						...
						if value and clanName == newClanName then
							...
							ToastNotif.new({
								text = "You've just got " .. clanName .. ", please leave the game and re-connect.",
							})
				...
				if not suc then
					if not waitingToTp then
						...
						ToastNotif.new({
							text = string.format("Rolled %d clans total. Your data is still rolled back.", rollCount),
						})
					end
