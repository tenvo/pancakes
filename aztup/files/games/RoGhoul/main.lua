...
local Players, ReplicatedStorage, TweenService, RunService, TeleportService =
	Services:Get("Players", "ReplicatedStorage", "TweenService", "RunService", "TeleportService")
...
do --//Utility
	...
	function autoFarmWorkings:queue(autoFarmType)
		...
		repeat
			wait()
		until not autoFarmWorkings:all(autoFarmType)
		...
		repeat
			...
			if Mob and MyRootPart and not autoFarmWorkings:all("autoFarm") then
				...
				until mobIsDead(Mob)
					...
					or not library.flags.toggleAutoFarm
				...
				if Tween.PlaybackState == Enum.PlaybackState.Completed then
					...
					repeat
						...
						if MyRootPart and mobRoot then
							createBodyVelocity(MyRootPart)
							MyRootPart.CFrame = CFrame.new(mobRoot.CFrame * (mobRoot.CFrame.LookVector * 2.5))
								* CFrame.new(0, 5, 0)
								* CFrame.Angles(math.rad(-90), 0, 0)

							if
								not (
									Character:FindFirstChild("Kagune")
									or Character:FindFirstChild("Katana")
									or Character:FindFirstChild("Quinque")
								)
							then
								FireServer(
									KaguneStages[library.flags.kaguneStage or 1],
									"Down",
									nil,
									"ShiftLock",
	...
	function toggleAutoTrainer(t)
		...
		repeat
			...
			local data = ReplicatedStorage.Remotes.Trainers.RequestTraining:InvokeServer(
				LocalPlayer.PlayerFolder.Trainers[PlayerTeam .. "Trainer"].Value
			)
	...
	function setDataloss(t)
		pcall(function()
			...
			originalFunctions.fireServer(
				ReplicatedStorage.Remotes.Settings.SpawnSelection,
				t and string.format("%s\128", currentSpawn) or currentSpawn
			)
