...
do -- // Functions
	...
	if
		typeof(load) ~= "table"
		or typeof(loadShared) ~= "table"
		or getrawmetatable(load)
		or getrawmetatable(loadShared)
	then
	...
	local function getOriginalFunction(f)
		...
		if not upvalues then
			return LocalPlayer:Kick("There was an error. DM Aztup [0]")
		end
	...
	do -- // Anti Ragdoll
		...
		oldEnterRagdoll = hookfunction(enterRagdoll, function(...)
			if library.flags.antiRagdoll then
				return
			end
			disableenvprotection()
	...
	do -- // Silent Aim
		...
		local function onCharacterAdded(character)
			...
			if not rootPart or destroyed then
				return
			end
		...
		upvalues.oldInitProjectile = hookfunction(
			initProjectile,
			function(self, projectileType, sharedData, info, callback, ...)
	...
	function autoGetUp(toggle)
		...
		repeat
			...
			if not playerCharacter:CanGetUp() then
				continue
			end
	...
	function instantBreakFree(toggle)
		...
		repeat
			...
			if not playerCharacter:CanBreakFree() then
				continue
			end
