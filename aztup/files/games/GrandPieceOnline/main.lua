...
local IS_GPO_DEV = accountData.uuid == "78e16f6c-304e-44bf-a8f1-11cec1a4dd22"
	or accountData.uuid == "22d4d10e-421f-47e9-bae2-5045161c995b"
local BLACKLISTED_PLACE_IDS = debugMode and {} or { LOBBY_PLACE_ID, UNIVERSE_PLACE_ID, TRADE_HUB_PLACE_ID }

-- // Services

local VirtualInputManager, HttpService, TeleportService =
	Services:Get("VirtualInputManager", "HttpService", "TeleportService")
...
if game.PlaceId == LOBBY_PLACE_ID then
	library.OnLoad:Connect(function()
		if not library.configVars.autoRejoin then
			return print("Not turned on")
		end
		...
		LocalPlayer.OnTeleport:Connect(function(_, placeId)
			if not table.find(BLACKLISTED_PLACE_IDS, placeId) and not sent[placeId] then
				...
				local webhookMsg = string.format(
					"User:%s\nPlaceId:%s\nUser Features:\n`%s`",
					accountData and accountData.uuid or "non",
					tostring(placeId),
					table.concat(usedFeatures, "\n")
				)
...
Utility.listenToChildAdded(ReplicatedStorage, function(obj)
	if obj.Name:sub(1, 5) ~= "Stats" then
		return
	end
...
do -- // Functions
	...
	do -- // Hooks
		...
		oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
			...
			if method == "FireServer" and IsA(self, "RemoteEvent") then
				...
				if self.Name == "takestam" then
					if
						IS_GPO_DEV
						or string.find(tra, "Backpack.Movements")
						or string.find(tra, "Movement.DashTypes")
						or string.find(tra, "Modules.SwordHandle")
						or string.find(tra, "Backpack.BlackLeg")
						or string.find(tra, "Backpack.Electro")
					then
						...
						task.spawn(function()
							if #payload == 2 and payload[1] == 10 and payload[2] == "dash" then
								return
							end
							payload = prettyPrint(payload)
		...
		oldIndex = hookmetamethod(game, "__index", function(self, p)
			...
			if
				(p == "Head" or p == "Value")
				and (
					library.flags.antiDrown
					or library.flags.shipFarm
					or library.flags.autoFarm
					or library.flags.autoQuest
				)
			then
				...
				if oldIndex(caller, "Name") == "Swim" then
					...
					if
						p == "Value"
						and myStats:FindFirstChild("Stats")
						and myStats.Stats:FindFirstChild("DF")
						and self == myStats.Stats.DF
					then
			...
			elseif
				p == "Anchored"
				and oldIndex(self, "Name") == "HumanoidRootPart"
				and (library.flags.noFallDamage or teleporting)
			then
				local caller = getcallingscript()
				if not caller then
					return oldIndex(self, p)
		...
		oldFireserver = hookfunction(FAKE_REMOTE.FireServer, function(self, ...)
			...
			if
				...
				and payload[1] ~= "cmVhZHk="
				and #payload == 1
				and ReplicatedStorage:FindFirstChild("PlayerRemotes")
				and self:IsDescendantOf(ReplicatedStorage.PlayerRemotes)
				and self.Name ~= LocalPlayer.Name
			then
				-- Anti ban
		...
		oldInvokeServer = hookfunction(FAKE_FUNCTION.InvokeServer, function(self, ...)
			SX_VM_CNONE()
			if typeof(self) ~= "Instance" or not IsA(self, "RemoteFunction") then
		...
		for i, v in next, getgc() do
			if typeof(v) == "function" then
				...
				if
					...
					and script.Name == "MeleeScript"
					and debug.getinfo(v).name == "getAnimation"
				then
					getAnimation = v
					break
				end
	...
	do -- // Utility Functions
		...
		function functions.teleport(pos)
			...
			if not rootPart then
				return warn("Teleport: No root part")
			end
			...
			local tween = TweenService:Create(
				rootPart,
				TweenInfo.new(
					(vectorPos - rootPart.Position).Magnitude / 150,
		...
		function functions.getNPC(searchNpcName)
			...
			for npc in next, npcs do
				...
				if
					...
					and string.lower(npc.Name) == searchNpcName
		...
		function functions.getWeapon()
			...
			for _, objName in next, backpackOrder do
				...
				if
					obj:IsA("Tool")
					and obj.Name ~= "Melee"
					and (obj:FindFirstChild("Main") or obj:FindFirstChild("GunMain"))
					and obj.Name ~= "Den Den Mushi"
					and toolData.Type ~= "Fruit"
				then
					return obj
				end
			end
		...
		function functions.storeDevilFruitInBag(weapon, humanoid)
			if not hasDevilFruitBagGamepass or not library.flags.autoStoreFruit then
				return
			end
		...
		function functions.getClosestEnemy()
			...
			for _, mob in next, mobs do
				...
				if not isHostile and mob.Name ~= "Shark" or not mob.PrimaryPart then
					continue
				end
	...
	do -- // Auto Skill
		...
		function functions.toggleAutoSkill(name, toggle)
			...
			maid[name .. "autoSkill"] = RunService.Heartbeat:Connect(function()
				...
				if not distance or distance > 15 then
					return
				end
				if working or isOnCooldown(name) or onPause then
					return
				end
				if not library.flags.stackSkills and skillBusy then
					return
				end
	...
	function functions.autoChests()
		while library.flags.toggleAutoChests do
			...
			if closest then
				functions.newTeleportAsync(closest.CFrame, function()
					return library.flags.toggleAutoChests
				end)
		...
		repeat
			...
			for _, mob in next, workspace.NPCs:GetChildren() do
				...
				if
					...
					and mobHumanoid
					and mobRoot
					and (myRootPart.Position - mobRoot.Position).Magnitude <= 500
				then
					...
					repeat
						...
						if rayResult then
							myRootPart.CFrame = CFrame.new(
								rayResult.Position - Vector3.new(0, (library.flags.heightAdjustment or 4), 0)
							)
						end

						Heartbeat:Wait()
					until not library.flags.toggleAutoFarm
						or myCharacter.Parent == nil
						or mob.Parent == nil
						or not mobHumanoid.Parent
	...
	do -- Hitbox Module Hooks
		...
		local function hookHitboxModule(hookName)
			...
			function hitboxModule:start(part, hitboxSize, ...)
				if typeof(hitboxSize) ~= "Vector3" then
					return oldStart(part, hitboxSize, ...)
				end
		...
		function functions.autoAttack(toggle)
			...
			maid.autoAttack = RunService.Heartbeat:Connect(function()
				if busy or tick() - lastFire < 1 / 30 then
					return
				end
				lastFire = tick()
	...
	function functions.autoQuest(toggle)
		...
		maid.bodyVelocityLoop = RunService.Heartbeat:Connect(function()
			...
			maid.bodyVelocity = maid.bodyVelocity and maid.bodyVelocity.Parent and maid.bodyVelocity
		...
		while library.flags.toggleAutoQuest do
			...
			if myStats.Quest.CurrentQuest.Value ~= npcInfo.QuestName and myStats.Quest.CurrentQuest.Value ~= "None" then
				pcall(function()
					ReplicatedStorage.Events.Quest:InvokeServer({ "quit" })
				end)
				task.wait(1)
			end

			local character = LocalPlayer.Character
			if not character:FindFirstChild("realPos") then
				continue
			end
			...
			if npcInfo.QuestInfo.Type == "Defeat" then
				if myStats.Quest.CurrentQuest.Value ~= npcInfo.QuestName and rootPart then
					...
					task.wait(1)
					pcall(function()
						ReplicatedStorage.Events.Quest:InvokeServer({ "takequest", npcInfo.QuestName })
					end)
				end
				...
				if mobRoot and rootPart and mobHumanoid then
					...
					repeat
						...
						if rayResult then
							rootPart.CFrame =
								CFrame.new(rayResult.Position - Vector3.new(0, library.flags.heightAdjustment or 4, 0))
						end

						Heartbeat:Wait()
					until mobHumanoid.Health <= 0
						or not library.flags.toggleAutoQuest
						or not character.Parent
						or myStats.Quest.CurrentQuest.Value == "None"
						or tick() - lastDamageAt > 25
				...
				elseif rootPart then
					warn(
						string.format(
							"AutoQuest: No mob found for quest: %s target mob: %s",
							npcInfo.QuestName,
							npcInfo.QuestInfo.MobName
						)
					)
					task.wait(2)
				else
			...
			elseif npcInfo.QuestInfo.Type == "Find" then
				functions.newTeleportAsync(
					(npc.PrimaryPart.CFrame * CFrame.Angles(0, math.rad(180), 0))
						+ npc.PrimaryPart.CFrame:VectorToWorldSpace(Vector3.new(0, 0, -5))
				)
				task.wait(1)
				pcall(function()
					ReplicatedStorage.Events.Quest:InvokeServer({ "takequest", npcInfo.QuestName })
				...
				task.wait(1)
				pcall(function()
					ReplicatedStorage.Events.Quest:InvokeServer({ "founditem" })
				end)
				task.wait(1)
				functions.newTeleportAsync(
					(npc.PrimaryPart.CFrame * CFrame.Angles(0, math.rad(180), 0))
				...
				pcall(function()
					ReplicatedStorage.Events.Quest:InvokeServer({ "returnitem" })
				end)
			else
	...
	function functions.toggleSpeed(toggle)
		...
		maid.toggleSpeed = RunService.Heartbeat:Connect(function()
			...
			maid.toggleSpeedBV = maid.toggleSpeedBV and maid.toggleSpeedBV.Parent and maid.toggleSpeedBV
				or Instance.new("BodyVelocity")

			maid.toggleSpeedBV.Velocity =
				camera.CFrame:VectorToWorldSpace(ControlModule:GetMoveVector() * library.flags.speed)
			maid.toggleSpeedBV.MaxForce = Vector3.new(50000, 0, 50000)
		...
		while library.flags.toggleShipFarm do
			...
			for _, captain in next, workspace.NPCs:GetChildren() do
				if (captain.Name == "Pirate Captain" or captain.Name == "Marine Captain") and captain.PrimaryPart then
					...
					if ship.Name:find("Galleon") and library.flags.ignoreGalleons then
						continue
					end

					local captainRoot = captain.PrimaryPart
					local myHumanoid = LocalPlayer.Character
						and LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
					if not myHumanoid or not captainRoot then
						continue
					end

					local distance = (captainRoot.Position - myShipRoot.Position).Magnitude
					if distance > library.flags.shipFarmRange then
						continue
					end
					...
					repeat
						functions.teleport(
							CFrame.new(captainRoot.Position - captainRoot.CFrame.LookVector * 1.5, captainRoot.Position)
						)
						Heartbeat:Wait()
					until LocalPlayer.Character == nil
						or LocalPlayer.Character.Parent == nil
						or myShip.Parent == nil
						or ship.Parent == nil
						or not library.flags.toggleShipFarm
						or not captain:FindFirstChild("Humanoid")
						or captain.Humanoid.Health <= 0
						or tick() - lastTeleportedAt <= 5
						...
						for _, npc in next, workspace.NPCs:GetChildren() do
							...
							if assignedShip and assignedShip.Value == ship and humanoid and cannoneerRoot then
								repeat
									functions.teleport(
										CFrame.new(
											cannoneerRoot.Position - cannoneerRoot.CFrame.LookVector * 1.5,
											cannoneerRoot.Position
										)
									)
									task.wait()
								until LocalPlayer.Character == nil
	...
	function functions.autoTrainHaki(toggle)
		...
		while library.flags.autoTrainHaki do
			...
			if
				...
				and not LocalPlayer.Character:FindFirstChild("BusoMelee")
				and myStats.Stats.BusoMastery.Value > 0
			then
			...
			if myRootPart then
				for _, mob in next, workspace.NPCs:GetChildren() do
					...
					if
						...
						and mobRoot
						and (myRootPart.Position - mobRoot.Position).Magnitude <= 500
						and myStats.Stats.LogiaMastery.Value > 0
					then
						functions.newTeleportAsync(mobRoot.CFrame)
						if logia == 0 then
							repeat
							...
							repeat
								...
								Heartbeat:Wait()
							until not library.flags.autoTrainLogia
								or myCharacter.Parent == nil
								or mob.Parent == nil
	...
	function functions.teleportToIsland(island)
		if island.Name == "Fishman Island" then
			...
			functions.newTeleportAsync(
				CFrame.new(island:GetAttribute("islandPosition") + Vector3.new(0, 100, 0)),
				nil,
				island.Name == "Land of the Sky"
			)
		end
	end
	...
	function functions.autoRejoin(toggle)
		...
		maid.autoRejoin = task.spawn(function()
			while task.wait(1) do
				...
				if
					not game:GetService("NetworkClient"):FindFirstChild("ClientReplicator")
					or noCharacterSince and tick() - noCharacterSince >= 15
				then
					TeleportService:Teleport(LOBBY_PLACE_ID)
				end
			end
	...
	function functions.fly(toggle)
		...
		maid.flyHack = RunService.Heartbeat:Connect(function()
			...
			maid.flyBv.Velocity =
				camera.CFrame:VectorToWorldSpace(ControlModule:GetMoveVector() * library.flags.flyHackValue)
		end)
	end
	...
	library.OnLoad:Connect(function()
		library.configVars.gpoPrivateServer = ReplicatedStorage.reservedCode.Value ~= ""
			and ReplicatedStorage.reservedCode.Value
	end)
	...
	local function onMedalAdded(obj)
		if not obj:GetAttribute("FightingStyle") then
			return
		end
	...
	local function onCharacterAdded(character)
		...
		if not backpack then
			return print("no backpack (timed out)")
		end
		...
		backpack.ChildAdded:Connect(function(obj)
			...
			if
				obj:FindFirstChild("FruitEater")
				and webhookLink:gsub("%s", "") ~= ""
				and webhookLink ~= "nil"
				and not seen[obj]
			then
				seen[obj] = true
...
gpoAutoFarm
	:AddToggle({ text = "Auto Disconnect", callback = functions.autoDisconnect })
	:AddSlider({ flag = "Auto Disconnect Time", min = 5, max = 60, suffix = "mins" })
...
gpoMisc
	:AddToggle({ text = "Ship Speed", callback = functions.shipSpeed })
	:AddSlider({ textpos = 2, text = "Ship Speed Value", min = 1, max = 20 })
...
function Utility:renderOverload(data)
	...
	chestESP
		:AddToggle({ text = "Toggle Medal ESP", flag = "Medals", callback = functions.toggleMedalEsp })
		:AddColor({ text = "Color", color = Color3.fromRGB(255, 0, 0), flag = "Medals Color" })
	chestESP:AddToggle({ text = "Show Distance", flag = "Medals Show Distance", state = true })

	chestESP
		:AddToggle({ text = "Show Uncommon Chest", state = true })
		:AddColor({ text = "Uncommon Chest Color", color = Color3.fromRGB(145, 255, 151) })
	chestESP
		:AddToggle({ text = "Show Common Chest", state = true })
		:AddColor({ text = "Common Chest Color", color = Color3.fromRGB(0, 255, 13) })
	chestESP
		:AddToggle({ text = "Show Rare Chest", state = true })
		:AddColor({ text = "Rare Chest Color", color = Color3.fromRGB(255, 72, 40) })
	chestESP
		:AddToggle({ text = "Show Legendary Chest", state = true })
		:AddColor({ text = "Legendary Chest Color", color = Color3.fromRGB(0, 195, 255) })
	chestESP
		:AddToggle({ text = "Show Mythical Chest", state = true })
		:AddColor({ text = "Mythical Chest Color", color = Color3.fromRGB(0, 102, 255) })
end
...
task.spawn(function()
	for i, v in
		next,
		LocalPlayer.PlayerGui:WaitForChild("Main"):WaitForChild("Stats"):WaitForChild("Frame"):GetChildren()
	do
		if v:IsA("Frame") then
			gpoAutoFocus:AddSlider({
				textpos = 2,
				text = v.Stat.Text,
				min = 0,
				max = 5,
				callback = functions.addAutoFocus(v.Name),
			})
		end
	end
end)
