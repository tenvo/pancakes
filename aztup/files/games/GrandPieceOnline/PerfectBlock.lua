...
if debugMode then
	...
	animLogger.OnClick:Connect(function(btnName, ctx)
		...
		elseif btnName == "Add To Ignore List" then
			if table.find(ignoredAnimations, ctx.animationId) then
				return
			end
...
local function parryAttack(timings, rootPart, animationTrack, maxRange, useAnimSpeed)
	...
	for i in next, timings do
		task.spawn(function()
			local convertedWait =
				calculatePingWait(getAllTimings(timings, i) / (useAnimSpeed and animationTrack.Speed or 1))
...
function PerfectBlock:_onHumanoidAdded()
	if not self._rootPart or not self._humanoid then
		return
	end
	...
	self._maid[humanoid] = humanoid.AnimationPlayed:Connect(function(animationTrack)
		...
		if not entityPos or not myRootPart then
			return
		end
		if (entityPos - myRootPart.Position).Magnitude >= 150 then
			return
		end
		...
		if library.flags.checkIfFacingTarget then
			...
			if dotProduct <= 0 then
				return print("Not parrying player is not facing target")
			end
		end
		...
		if library.flags.checkIfTargetFaceYou then
			...
			if dotProduct <= 0 then
				return print("Not parrying target is not facing player")
			end
		end
		...
		if debugMode and not table.find(ignoredAnimations, animId) then
			if not animName then
				...
				task.spawn(function()
					animationNames[animId] =
						MarketplaceService:GetProductInfo(tonumber(animId), Enum.InfoType.Asset).Name
				end)
			end
			...
			animLogger:AddText({
				animationId = animId,
				text = string.format(
					"Animation played %s | %s from %s %s",
					animId,
					animationNames[animId],
					self._name,
					waitTime and "has" or "dont have"
				),
			})
		end
