...
do -- // Functions
	...
	function saveHouse(player)
		...
		for _, wall in next, plot.House.Walls:GetChildren() do
			if wall.Name ~= "Poles" then
				local offSetFrom, offSetTo =
					ground.CFrame:PointToObjectSpace(getPolePosition(wall.BPole)),
					ground.CFrame:PointToObjectSpace(getPolePosition(wall.FPole))
		...
		for _, path in next, plot.House.Paths:GetChildren() do
			if path.Name ~= "Poles" then
				local offSetFrom, offSetTo =
					ground.CFrame:PointToObjectSpace(getPolePosition(path.BPole)),
					ground.CFrame:PointToObjectSpace(getPolePosition(path.FPole))
				local floorData = {}
		...
		for _, fence in next, plot.House.Fences:GetChildren() do
			if fence.Name ~= "Poles" then
				...
				local offSetTo, offSetFrom =
					ground.CFrame:PointToObjectSpace(to), ground.CFrame:PointToObjectSpace(from)
				local fenceData = {}
	...
	do -- // Remote Spy
		...
		local blacklistedTypes = {
			...
			"VehicleUpdate",
			"FloorPos",
			"RequestLockerDataForSpecificLocker",
		}

		oldFireServer = hookfunction(network.FireServer, function(self, data, ...)
			if data.Type == "EndShift" and library.flags.pizzaDelivery then
				return
			end
	...
	local function tweenTeleport(position)
		...
		for i, v in next, waypoints do
			...
			local tween =
				TweenService:Create(cfValue, tweenInfo, { Value = CFrame.new(v.Position + Vector3.new(0, 4, 0)) })
	...
	function stylezHairDresser(toggle)
		...
		repeat
			if jobManager:GetJob() == "StylezHairdresser" then
				local workstation =
					findCurrentWorkstation(GetLocation("StylezHairStudio").Interior.HairdresserWorkstations)
				if workstation then
					if workstation.Mirror:FindFirstChild("HairdresserGUI") then
						workstation.Mirror.HairdresserGUI.Overlay:FindFirstChild("false").ImageRectOffset =
							Vector2.new(0, 0)
						workstation.Mirror.HairdresserGUI.Overlay:FindFirstChild("false").ImageColor3 =
							Color3.new(0, 255, 0)
				...
				for i, workstation in next, GetBurgerWorkstations() do
					if workstation then
						if workstation.OrderDisplay.DisplayMain:FindFirstChild("CashierGUI") then
							workstation.OrderDisplay.DisplayMain.CashierGUI.Overlay:FindFirstChild("false").ImageRectOffset =
								Vector2.new(0, 0)
							workstation.OrderDisplay.DisplayMain.CashierGUI.Overlay:FindFirstChild("false").ImageColor3 =
								Color3.new(0, 255, 0)
							for i, v in
								next,
								getconnections(workstation.OrderDisplay.DisplayMain.CashierGUI.Frame.Done.Activated)
							do
								v.Function()
							end
	...
	function FindClosestEmptyShelf()
		...
		for i, v in next, GetLocation("Supermarket").Interior.Shelves:GetChildren() do
			if
				LocalPlayer:DistanceFromCharacter(v.PrimaryPart.Position) < closestDistance
				and v.IsEmpty.Value == true
			then
	...
	local function Fish()
		...
		if LocalPlayer.Character:FindFirstChild("Fishing Rod") then
			...
			if LocalPlayer.Character:FindFirstChild("Fishing Rod") then
				...
				con = LocalPlayer.Character["Fishing Rod"].Bobber
			...
			if jobManager:GetJob() == "PizzaPlanetBaker" then
				...
				if workstation then
					...
					elseif order.Value ~= "true" then
						if
							workstation:FindFirstChild("OrderDisplay")
							and workstation.OrderDisplay:FindFirstChild("DisplayMain")
							and workstation.OrderDisplay.DisplayMain:FindFirstChild("BakerGUI")
						then
							workstation.OrderDisplay.DisplayMain.BakerGUI.Overlay:FindFirstChild("false").ImageRectOffset =
								Vector2.new(0, 0)
							workstation.OrderDisplay.DisplayMain.BakerGUI.Overlay:FindFirstChild("false").ImageColor3 =
								Color3.new(0, 255, 0)
							for i, v in
								next,
								getconnections(workstation.OrderDisplay.DisplayMain.BakerGUI.Frame.Done.Activated)
							do
								v.Function()
							end
						end
	...
	function NextCustomer()
		if jobManager:GetJob() == "SupermarketCashier" and library.flags.supermarketCashier then
			...
			repeat
				...
				task.wait()
			until jobManager:GetJob() ~= "SupermarketCashier"
				or Station.Occupied.Value ~= nil
					and (Station.Occupied.Value.Head.Position - Station.CustomerTarget_2.Position).magnitude < 3
			network:FireServer({
				Type = "JobCompleted",
				Workstation = Station,
			})
	...
	function pizzaDelivery(toggle)
		...
		repeat
			...
			repeat
				...
				local box = GetLocation("PizzaPlanet")
					:WaitForChild("Interior")
					:WaitForChild("Conveyor")
					:WaitForChild("MovingBoxes")
					:FindFirstChildOfClass("UnionOperation")
				if not box then
					continue
				end
			...
			if jobManager:GetJob() == "LumberWoodcutter" then
				...
				if Tree then
					local tweenInfo = TweenInfo.new(
						(LocalPlayer.Character.HumanoidRootPart.Position - Tree.Position).Magnitude / 45,
						Enum.EasingStyle.Linear
					)
					local tween =
						TweenService:Create(LocalPlayer.Character.HumanoidRootPart, tweenInfo, { CFrame = Tree.CFrame })
					...
					repeat
						...
						task.wait()
					until (Tree.Position.Y < 0)
						or (jobManager:GetJob() ~= "LumberWoodcutter")
						or (library.flags.lumber == false)
				end
			end
	...
	local function FindClosestStone()
		...
		for i, v in next, workspace.Environment.Locations.Static_MinerCave.Folder:GetChildren() do
			if
				LocalPlayer:DistanceFromCharacter(v.PrimaryPart.Position) < closestDistance
				and v.PrimaryPart.Position.Y < closestY
			then
	...
	local function FindClosestOre()
		...
		for i, v in next, workspace.Environment.Locations.Static_MinerCave.Folder:GetChildren() do
			if v:FindFirstChild("M") then
				if
					table.find(ColorsValue, bestColor) < table.find(ColorsValue, v:FindFirstChild("M").BrickColor.Name)
				then
					closestBlock = v
					bestColor = v:FindFirstChild("M").BrickColor.Name
				elseif
					table.find(ColorsValue, bestColor)
					== table.find(ColorsValue, v:FindFirstChild("M").BrickColor.Name)
				then
	...
	function miner(toggle)
		...
		repeat
			if
				library.flags.miner
				and jobManager:GetJob() == "CaveMiner"
				and workspace.Environment.Locations:FindFirstChild("Static_MinerCave")
			then
	...
	local function GetMotorWorkstation()
		if GetLocation("MikesMoters") then
			...
			for i, v in next, GetLocation("MikesMoters").Interior.MechanicWorkstations:GetChildren() do
				if
					v:FindFirstChild("InUse")
					and v:FindFirstChild("Occupied")
					and v.InUse.Value == nil
					and v.Occupied.Value ~= nil
				then
					Station = v
				end
			end
						...
						if Order:FindFirstChild("Oil") then
							...
							if Oil ~= nil then
								repeat
									...
									network:FireServer({
										Type = "TakeOil",
										Object = GetLocation("MikesMoters").Interior.OilCans:FindFirstChildWhichIsA(
											"Model"
										),
									})
									task.wait()
								...
								repeat
									task.wait()
								until (not LocalPlayer.Character:FindFirstChild("Oil Can"))
									or not library.flags.mechanic
								network:FireServer({
									Type = "JobCompleted",
									Workstation = v,
								})
						...
						elseif Order:FindFirstChild("Wheels") then
							...
							for i = 1, 2 do
								repeat
									...
									network:FireServer({
										Type = "TakeWheel",
										Object = GetLocation("MikesMoters").Interior.TireRacks:FindFirstChild(
											WheelType
										),
									})
									task.wait()
								until LocalPlayer.Character:FindFirstChild(WheelType .. " Wheel")
									or not library.flags.mechanic
								tweenTeleport(v.Display.Screen.Position + Vector3.new(0, 0, 5), 12)
								if i == 1 then
									network:FireServer({
								...
								repeat
									task.wait()
								until (not LocalPlayer.Character:FindFirstChild(WheelType .. " Wheel"))
									or not library.flags.mechanic
								if i == 2 then
									network:FireServer({
										Type = "JobCompleted",
						...
						elseif Order:FindFirstChild("Color") then
							...
							if ColorValue ~= nil then
								repeat
									...
									network:FireServer({
										Type = "TakePainter",
										Object = GetLocation("MikesMoters").Interior.PaintingEquipment:FindFirstChild(
											ColorValue
										),
									})
									task.wait()
								until LocalPlayer.Character:FindFirstChild("Spray Painter")
									or not library.flags.mechanic
								tweenTeleport(v.Display.Screen.Position + Vector3.new(0, 0, 5), 12)
								network:FireServer({
									Type = "FixBike",
								...
								repeat
									task.wait()
								until (not LocalPlayer.Character:FindFirstChild("Spray Painter"))
									or not library.flags.mechanic
								network:FireServer({
									Type = "JobCompleted",
									Workstation = v,
	...
	local function FindClosestTrash()
		...
		for i, v in next, workspace.Environment.Locations.GreenClean.Spawns:GetChildren() do
			if
				LocalPlayer:DistanceFromCharacter(v.Position) < closestDistance
				and v:FindFirstChildWhichIsA("Decal", true)
			then
				closestDistance = LocalPlayer:DistanceFromCharacter(v.Position)
				closestBlock = v
			end
	...
	function GetNearPlayers(Position, MaxDistance)
		...
		for i, v in pairs(Players:GetPlayers()) do
			if v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v ~= LocalPlayer then
				...
				if Distance <= MaxDistance then
					-- Raycast from our position to target HRP
					...
					-- True if nothing blocks the view OR the first hit is the target character
					if not rayResult or rayResult.Instance:IsDescendantOf(v.Character) then
						return true
					end
				end
			end
	...
	function RadiusVisualizerCallback(t)
		if t then
			if library.flags.radiusCleanup ~= nil then
				return
			end
			library.flags.radiusCleanup = true

			local root = LocalPlayer.Character:WaitForChild("HumanoidRootPart")

			-- CONFIG
			--getgenv().TARGET_RADIUS = 100        -- desired radius in studs (you can change this at runtime)
			local SMOOTHNESS = 10 -- higher -> snappier
			local SEGMENTS = 64 -- more = smoother circle (16-128)
			local SEGMENT_THICKNESS = 0.3 -- width of each segment (radial thickness)
			local SEGMENT_HEIGHT = 0.15 -- vertical thickness (height above ground)
			local OVERLAP_FACTOR = 1.1 -- multiply segment length by this to avoid gaps
			local COLOR = Color3.fromRGB(0, 150, 255)
			local TRANSPARENCY = 0.15
			local Y_OFFSET = 3.0 -- how far below HRP the ring sits (adjust for your character height)
			local PARENT = workspace -- where to parent the parts
			------------------------------------------------

			-- Create container model
			local model = Instance.new("Model")
			model.Name = "AOERingModel_" .. LocalPlayer.Name
			model.Parent = PARENT

			-- Pre-create segment parts
			local segments = {}
			for i = 1, SEGMENTS do
				...
				p.Size = Vector3.new(1, SEGMENT_HEIGHT, SEGMENT_THICKNESS) -- will be scaled/positioned
				p.Parent = model
				table.insert(segments, p)
			end

			-- Current radius used for smooth scaling
			local currentRadius = 0

			-- helper: compute segment length to fill circumference
			local function computeSegmentLength(radius)
				local circumference = 2 * math.pi * radius
				local baseLength = circumference / SEGMENTS
				return baseLength * OVERLAP_FACTOR
			end
			...
			RunService.RenderStepped:Connect(function(dt)
				...
				-- Smooth lerp toward TARGET_RADIUS (frame-rate independent)
				local alpha = 1 - math.exp(-SMOOTHNESS * dt)
				currentRadius = currentRadius + (library.flags.safeRadius - currentRadius) * alpha
				if currentRadius < 0.01 then
					currentRadius = 0.01
				end

				-- compute per-segment length and placement
				local segLen = computeSegmentLength(currentRadius)
				-- We'll orient each segment so its long axis follows the tangent of the circle.
				for i, part in ipairs(segments) do
					local angle = ((i - 1) / SEGMENTS) * (2 * math.pi)
					local cosA = math.cos(angle)
					local sinA = math.sin(angle)

					-- center position for this segment (on circle)
					local cx = root.Position.X + cosA * currentRadius
					local cz = root.Position.Z + sinA * currentRadius
					local cy = root.Position.Y - Y_OFFSET

					-- place segment centered slightly outward so it forms a ring outline
					local pos = Vector3.new(cx, cy, cz)

					-- length along tangent direction
					part.Size = Vector3.new(segLen, SEGMENT_HEIGHT, SEGMENT_THICKNESS)

					-- compute tangent direction (perpendicular to radius)
					local tangent = Vector3.new(-sinA, 0, cosA).Unit

					-- build CFrame: position + rotation where part's X is along tangent and Z is thickness
					-- We want the part's front (its +X) to point along tangent. Part orientation: X-right, Y-up, Z-back.
					local right = tangent
					local up = Vector3.new(0, 1, 0)
					local back = right:Cross(up) -- Z axis for part
					local cframe = CFrame.fromMatrix(pos, right, up, back)

					part.CFrame = cframe
				end
			end)

			-- Clean up when character is removed or player leaves
			...
			library.flags.radiusCleanup = cleanup
		else
			if typeof(library.flags.radiusCleanup) == "function" then
				library.flags.radiusCleanup()
				task.wait()
				library.flags.radiusCleanup = nil
			end
		end
	end

	function AutoDisable(toggle)
		if not toggle then
			return
		end
		...
		repeat
			...
			then
				if
					not FlagCache
					and (GetNearPlayers(LocalPlayer.Character.HumanoidRootPart.Position, library.flags.safeRadius))
				then
					FlagCache = library:Set(AutofarmFlags, false)
				elseif
					FlagCache
					and (not GetNearPlayers(LocalPlayer.Character.HumanoidRootPart.Position, library.flags.safeRadius))
				then
					library:Set(FlagCache, true)
					FlagCache = nil
				end
			end
		until not library.flags.autoDisable
	end
...
for _, o in pairs(Autofarm.options) do
	table.insert(AutofarmFlags, o.flag)
end
...
Autobuild:AddList({
	...
	noload = true,
	playerOnly = true,
	callback = copyPlayerHousePrompt,
})
...
Autobuild:AddList({
	...
	noload = true,
	values = {},
	callback = loadPlayerHousePrompt,
})

Misc:AddList({
	text = "Teleport To Player Plot",
	skipflag = true,
	noload = true,
	playerOnly = true,
	callback = teleportToPlayerPlot,
})

Misc:AddToggle({
	text = "Auto Disable",
	tip = "disables and resumes when the players are away from the radius",
	callback = AutoDisable,
}):AddSlider({ text = "Safe Radius", min = 1, float = 1, max = 150, value = 30 })
Misc:AddToggle({ text = "Radius Visualizer", callback = RadiusVisualizerCallback })
