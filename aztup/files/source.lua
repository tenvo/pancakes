...
local Players, TeleportService, HttpService, ReplicatedStorage, MarketplaceService, CoreGui, TweenService, VirtualInputManager =
	Services:Get(
		getServerConstant("Players"),
		"TeleportService",
		"HttpService",
		"ReplicatedStorage",
		"MarketplaceService",
...
getgenv().originalFunctions = {
	...
	index = executor.getrawmetatable(game).__index,
...
if not umarlib then
	...
	do -- // KeyBinds
		...
		for _, v in next, library.options do
			if
				(v.type == "toggle" or v.type == "button")
				and v.section
				and not table.find(blacklistedSections, v.section.title)
			then
		...
		options:AddButton({
			...
			callback = function()
				if
					library:ShowConfirm(
						'Are you sure you want to reset <font color="rgb(255, 0, 0)">all</font> keybinds?'
					)
				then
