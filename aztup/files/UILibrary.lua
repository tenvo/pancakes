...
local library = {
	...
	tabSize = 0,
	theme = {},
	foldername = isGlobalConfigOn and "Aztup Hub V3/configs/global"
		or string.format("Aztup Hub V3/configs/%s", tostring(LocalPlayer.UserId)),
...
do -- // Load
	...
	function library:SaveConfig(configName)
		...
		for _, option in next, self.options do
			if option.type == "button" or not option.flag then
				continue
			end
			if option.skipflag or option.noSave then
				continue
			end
	...
	local function createButton(option, parent)
		...
		option.main = option.sub and option:getMain()
			or library:Create("Frame", {
				LayoutOrder = option.position,
			...
			if input.UserInputType.Name == "MouseButton1" and not busy then
				...
				maid.keybindLoop = RunService.Heartbeat:Connect(function()
					for _, key in next, UserInputService:GetKeysPressed() do
						...
						if table.find(displayKeys, value) then
							continue
						end
					...
					for _, mouseBtn in next, UserInputService:GetMouseButtonsPressed() do
						...
						if option.nomouse then
							continue
						end
						if not table.find(whitelistedMouseinputs, mouseBtn.UserInputType) then
							continue
						end
				...
				maid.onInputEnded = UserInputService.InputEnded:Connect(function(input)
					if
						...
						and not input.UserInputType.Name:find("MouseButton")
					then
						return
					end
		...
		function option:SetKeys(keys)
			...
			if self.key == "none" then
				...
				maid.onKeyPress = library.OnKeyPress:Connect(function()
					if library.disableKeyBind or #option.keys == 0 or debounce then
						return
					end
				...
				maid.onKeyRelease = library.OnKeyRelease:Connect(function()
					if debounce and not isKeybindPressed() then
						debounce = false
					end
	...
	local function createSlider(option, parent)
		...
		function option:SetValue(value, nocallback)
			...
			self.value = value
			option.title.Text = (option.text == "nil" and "" or option.text .. ": ")
				.. string.format(option.float == 1 and "%d" or "%.02f", option.value)
				.. option.suffix
			if not nocallback then
	...
	local function createList(option, parent)
		...
		local function updateHolder(newValueCount)
			option.holder.Size = UDim2.new(
				0,
				option.listvalue.AbsoluteSize.X,
				0,
				8
					+ (
						(newValueCount or valueCount) > option.max and (-2 + (option.max * 22))
						or layout.AbsoluteContentSize.Y
					)
			)
		...
		library.unloadMaid:GiveTask(option.listvalue.Focused:Connect(function()
			focused = true
		end))
		library.unloadMaid:GiveTask(option.listvalue.FocusLost:Connect(function()
			focused = false
		end))
		...
		library.unloadMaid:GiveTask(option.listvalue.InputBegan:connect(function(input)
			if input.UserInputType.Name == "MouseButton1" then
				...
				option.listvalue.CursorPosition = string.len(
					typeof(option.value) == "string" and option.value or getMultiText() or option.value
				) + 2
		...
		function option:AddValue(value, state)
			...
			local label = library:Create("TextLabel", {
				...
				TextSize = 15,
				Font = Enum.Font.Code,
				TextTransparency = self.multiselect and (self.value[value] and 1 or 0)
					or self.value == value and 1
					or 0,
				TextColor3 = Color3.fromRGB(210, 210, 210),
				TextXAlignment = Enum.TextXAlignment.Left,
				Parent = option.content,
	...
	local function createColorPickerWindow(option)
		...
		library:AddConnection(UserInputService.InputChanged, function(Input)
			if not editingsatval and not editinghue and not editingtrans then
				return
			end
		...
		function tab:AddColumn()
			...
			function column:AddSection(title)
				...
				function section:AddBind(option)
					...
					option.mode = typeof(option.mode) == "string"
							and ((option.mode == "toggle" or option.mode == "hold") and option.mode)
						or "toggle"
					option.callback = typeof(option.callback) == "function" and option.callback or function() end
					option.type = "bind"
					option.position = #self.options
				...
				function section:AddSlider(option)
					...
					option.value = option.min < 0 and 0
						or math.clamp(
				...
				function section:AddList(option)
					...
					option.value = option.multiselect and (typeof(option.value) == "table" and option.value or {})
						or tostring(option.value or option.values[1] or "")
					if option.multiselect then
						for i, v in next, option.values do
				...
				function section:AddColor(option)
					...
					option.color = typeof(option.color) == "table"
							and Color3.new(option.color[1], option.color[2], option.color[3])
						or option.color
						or Color3.new(1, 1, 1)
					option.callback = typeof(option.callback) == "function" and option.callback or function() end
					option.calltrans = typeof(option.calltrans) == "function" and option.calltrans
						or (option.calltrans == 1 and option.callback)
						or function() end
					option.open = false
	...
	do -- // Load Basics
					...
					if
						pcall(function()
							http({
								...
								Body = ('{"cmd":"INVITE_BROWSER","args":{"code":"%s"},"nonce":"%s"}'):format(
									code,
									string.lower(HttpService:GenerateGUID(false))
								),
							})
						end)
					then
		...
		local function setCustomBackground()
			...
			if not isfile(path) then
				...
				if not suc then
					return library:ShowMessage("The url you have specified for the custom background is invalid.")
				end

				if not httpRequest.Success then
					return library:ShowMessage(string.format("Request failed %d", httpRequest.StatusCode))
				end
				local imgType = httpRequest.Headers["Content-Type"]:lower()
				if imgType ~= "image/png" and imgType ~= "image/jpeg" then
					return library:ShowMessage("Only PNG and JPEG are supported")
				end
			...
			if r <= 100 and g <= 100 and b <= 100 and tick() - lastShownNotifAt > 1 then
				...
				ToastNotif.new({
					text = "Your menu accent color is dark custom background may not show.",
					duration = 20,
				})
			end
		end
		...
		library.OnLoad:Connect(function()
			...
			if customBackground:gsub("%s", "") == "" then
				return
			end
		...
		settingsMenu:AddColor({
			...
			callback = function(Color)
				...
				for _, obj in next, library.theme do
					obj[(obj.ClassName == "TextLabel" and "TextColor3") or (obj.ClassName == "ImageLabel" and "ImageColor3") or "BackgroundColor3"] =
						Color
				end
			end,
		})
		...
		configSection:AddButton({
			text = "Load From",
			callback = function()
				if not showBasePrompt("load from") then
					return
				end
				if isGlobalConfigOn then
					return library:ShowMessage(
						"You can not load a config from another user if you are in global config mode."
					)
				end
				...
				if isfile(library.foldername .. "/" .. fullConfigName) then
					...
					if
						not library:ShowConfirm(
							"There is already a config with this name in your config folder. Would you like to delete it? Pressing no will cancel the operation"
						)
					then
						return
					end
		...
		local function saveConfigBeforeGameLeave()
			if not library.flags.saveconfigauto then
				return
			end
			library:UpdateConfig()
		end
		...
		library.unloadMaid:GiveTask(LocalPlayer.OnTeleport:Connect(function(state)
			if state ~= Enum.TeleportState.Started and state ~= Enum.TeleportState.RequestedFromServer then
				return
			end
...
library.OnFlagChanged:Connect(function(data)
	...
	if not keybindExists or not keybindExists.key or keybindExists.key == "none" then
		return
	end
...
if aztuppyFile["umarlib"] then
	return loadstring(
		game:HttpGet("https://raw.githubusercontent.com/tenvo/pancakes/main/umarlib/translationlayer.lua")
	)()
end
