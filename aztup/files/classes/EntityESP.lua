...
do --// Entity ESP
	...
	function EntityESP:Update(t)
		...
		if isTeamMate and not showTeam then
			return self:Hide()
		end
		...
		local label, box, line, healthBar, healthBarValue =
			self._label, self._box, self._line, self._healthBar, self._healthBarValue
		local pluginData = self:Plugin()

		local text = "["
			.. (pluginData.playerName or self._playerName)
			.. "] ["
			.. mathFloor(distance)
			.. "]\n["
			.. mathFloor(health)
		...
		setRP(
			label,
			"Position",
			Vector2New(labelPos.X, labelPos.Y - realGetRPProperty(self._labelObject, "TextBounds").Y)
		)
		...
		if showHealthBar then
			...
			local healthBarValueTopRight = worldToViewportPoint(
				camera,
				rootPartPosition + healthBarValueOffsetTopRight - self:ConvertVector(0, healthBarValueHealth, 0)
			)
			local healthBarValueBottomLeft =
				worldToViewportPoint(camera, rootPartPosition - healthBarValueOffsetBottomLeft)
