...
function Notification:_init()
	...
	local t = self:_tweenProperty(
		self._progressBar,
		positionStr,
		framePos + Vector2.new(0, self._frame.Size.Y - 3),
		TWEEN_INFO,
		true
	) --We dont really want this to be cancelable
	...
	self._maid._progressConnection = t.Completed:Connect(function() --This should prob use maids lol
		if self._options.duration then
			...
			self:_tweenProperty(
				self._progressBar,
				positionStr,
				framePos - Vector2.new(-self._frame.Size.X, -(self._frame.Size.Y - 3)),
				TweenInfo.new(self._options.duration, Enum.EasingStyle.Linear)
			) --You should technically remove this after its complete but doesn't matter
		end
	end)
end
...
function Notification:MouseInFrame()
	...
	return (mousePos.X >= framePos.X and mousePos.X <= bottomRight.X)
		and (mousePos.Y >= framePos.Y and mousePos.Y <= bottomRight.Y)
end

function Notification:GetHovered()
	for _, notif in next, Notifications do
		if notif:MouseInFrame() then
			return notif
		end
	end
...
function Notification:MoveUp() --Going to use this to move all the drawing instances up one
	...
	for i, v in next, Notifications do
		...
		if not v._options.duration then
			continue
		end
		...
		v:_tweenProperty(
			v._progressBar,
			positionStr,
			newFramePos - Vector2.new(-v._frame.Size.X, -(v._frame.Size.Y - 3)),
