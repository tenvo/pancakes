...
local UserInputService, TweenService, TextService, ReplicatedStorage, Players, HttpService =
	Services:Get("UserInputService", "TweenService", "TextService", "ReplicatedStorage", "Players", "HttpService")
...
do
	...
	function Text:SetText(text)
		...
		local textSize = TextService:GetTextSize(
			self.label.ContentText,
			20,
			Enum.Font.Roboto,
			Vector2.new(self._parent._logs.AbsoluteSize.X, math.huge)
		)
...
function TextLogger.new(params)
	...
	local function makeButton(btnName)
		...
		button.InputBegan:Connect(function(inputObject, gpe)
			if gpe or inputObject.UserInputType ~= Enum.UserInputType.MouseButton1 then
				return
			end
...
function TextLogger:UpdateCanvas()
	for _, v in next, self.logs do
		local textSize = TextService:GetTextSize(
			...
			20,
			Enum.Font.Roboto,
			Vector2.new(self._logs.AbsoluteSize.X, math.huge)
		)
