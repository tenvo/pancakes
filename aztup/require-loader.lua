...
local function QueryGame(query)
	...
	if typeof(GameScripts) ~= "table" then
		return warn("[ERROR] couldn't load games directory")
	end
...
local function customRequire(url, useHigherLevel)
	...
	if not lhost and not url then
		lhost = shared.aztuppy.root
		url = rawurl
	end
	...
	elseif
		not table.find({ "source.lua", "UILibrary.lua", "metadata.json" }, url) and not shared.aztuppy["payload"]
	then
		requestData = httpRequest({
			Url = QueryGame(url),
		})
		...
		if lhost == shared.aztuppy.root and shared.aztuppy["payload"] then
			if
				not table.find({ "source.lua", "UILibrary.lua", "metadata.json" }, url)
				and shared.aztuppy["payload"]._root ~= nil
			then
				lhost = shared.aztuppy["payload"]._root
			end
		end
...
local function GAMES_SETUP()
	...
	if not gameName and not aztuppy["payload"] then
		return warn("no custom game for this game")
	end
	if aztuppy["payload"] then
		aztuppy.payload()
		return print("[Payload] Finished Call")
	end
