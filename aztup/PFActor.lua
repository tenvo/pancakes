...
do
	...
	function characterObject:setBaseWalkSpeed(...)
		if not char or self ~= char then
			return oldSetBaseWalkspeed(self, ...)
		end
		local args = { ... }
...
local weaponVars = {
	...
	InstantReload = function(req)
		...
		if not req.animations then
			return
		end
		for k, x in next, req.animations do
			if
				(k:lower():find("reload") or k:lower():find("pullbolt") or k:lower():find("onfire"))
				and type(x) == "table"
			then
				print("passed")
				x.timescale = 0
...
local function applyGunModifications(editType, isEnabled)
	...
	local function onGunUpdate()
		...
		for _, weaponData in next, weaponDatas do
			...
			if not vars then
				warn("[WeaponController] no vars for", editType)
				continue
			end
...
oldIndex = hookmetamethod(game, "__index", function(self, p)
	...
	if p == "CFrame" then
		...
		if currentGun and (self == currentGun._barrelPart or oldIndex(self, "Name") == "SightMark") then
			if aimPart then
				local _, TimeToHit =
					physics.trajectory(self.Position, aimPart.Position, currentGun:getWeaponStat("bulletspeed"))
	...
	if flags.bulletTracers and args[1] == "newbullets" then
		...
		task.spawn(function()
			...
			for i = 1, #args[2].bullets do
				task.spawn(
					bulletTracer,
					gun._barrelPart.Position,
					(gun._barrelPart.Position + args[2].bullets[i][1]),
					flags.bulletTracersLifetime
				)
			end
		end)
	end
...
particle.new = function(data, ...)
	...
	if aimPart and data.visualorigin == currentGun._barrelPart.Position then
		...
		data.velocity =
			physics.trajectory(currentGun._barrelPart.Position, data.position, currentGun:getWeaponStat("bulletspeed"))
	end
