...
function Library:Window(winName, mainColor, hideBind)
		...
		task.spawn(function()
			pcall(function()
				...
				elseif which == "pos" then
					...
					TweenService
						:Create(gui, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), UDimStuff)
						:Play()
				end
			end)
		end)
	...
	local function dragify(Frame)
		dragToggle = nil
		dragSpeed = 0.25 -- You can edit this.
		dragInput = nil
		dragStart = nil
		dragPos = nil
	...
	function Tabs:Tab(tabName)
		...
		button.MouseButton1Click:Connect(function()
			...
			for _, v in pairs(tabH:GetChildren()) do
				if v:IsA("Frame") and v.Name ~= "seperator" then
					pcall(function()
						...
						task.spawn(function()
							Tween("Transparency", stroke, { Transparency = 1 }, 0.3)
							Tween("ImageTransparency", v.GlowTab, { ImageTransparency = 1 }, 0.3)
							Tween("BackgroundTransparency", v, { BackgroundTransparency = 1 }, 0.3)
						end)
					end)
				end
			end
			...
			Tween("Transparency", Color_5, { Transparency = 0.2 }, 0.3)
			Tween("ImageTransparency", GlowTab, { ImageTransparency = 0 }, 0.3)
			Tween("BackgroundTransparency", tabButton, { BackgroundTransparency = 0 }, 0.3)
			TabFrame.Visible = true
		end)
		...
		function Elements:Button(buttonName, callback)
			...
			function buttonFunc:SetText(newTxt)
				Button.Text = newTxt
			end
		...
		function Elements:Toggle(toggleName, currentState, callback)
			...
			if state then
				task.spawn(function()
					toggleDebounce = true
					Tween("pos", Ball, UDim2.new(0.552, 0, 0, 0), 0.3)
					Tween("BackgroundColor", Ball, { BackgroundColor3 = mainColor }, 0.3)
					task.wait(0.3)
			...
			local function ToggleOn()
				...
				task.spawn(function()
					...
					task.delay(0.01, function()
						repeat
							task.wait()
						until Ball.BackgroundColor3 ~= nil
						Tween("BackgroundColor", Ball, { BackgroundColor3 = mainColor }, 0.3)
					end)
					task.wait(0.3)
					toggleDebounce = false
			...
			local function ToggleOff()
				...
				task.spawn(function()
					toggleDebounce = true
					Tween("pos", Ball, UDim2.new(-0.00882184505, 0, 0, 0), 0.3)
					Tween("BackgroundColor", Ball, { BackgroundColor3 = Color3.fromRGB(67, 67, 67) }, 0.3)
					task.wait(0.3)
					toggleDebounce = false
				end)
		...
		function Elements:Slider(sliderName, config, callback)
			...
			SB.MouseButton1Down:Connect(function()
				...
				local function updateSlider(input)
					...
					local SValue = math.floor(
						(((tonumber(max) - tonumber(min)) / NormalSizeX) * SF.AbsoluteSize.X) + tonumber(min)
					)
					SV.Text = SValue
			...
			function sliderFunc:SetValue(desiredValue)
				if tonumber(desiredValue) and desiredValue <= max and desiredValue >= min then
					...
					local DefaultScale = (SliderDef - min) / (max - min)
					Tween("size", SF, UDim2.fromScale(DefaultScale, 1), 0.3)
		...
		function Elements:Dropdown(dropName, list, callback)
			...
			local function ToggleDropdown()
				...
				if not opened then
					task.spawn(function()
						...
						Tween("size", DropdownList, UDim2.new(0, 424, 0, 98), 0.15)
						Tween("rotation", ImageLabel, { Rotation = 180 }, 0.3)
						task.wait(0.3)
				...
				elseif opened then
					task.spawn(function()
						...
						Tween("size", DropdownList, UDim2.new(0, 424, 0, 0), 0.15)
						Tween("rotation", ImageLabel, { Rotation = 0 }, 0.3)
						task.wait(0.3)
						DropdownList.Visible = false
						opened = false
						dropDebounce = false
			...
			local info =
				{ Name = dropName, Type = "Dropdown", Value = dropdownFunc:GetChoice(), Element = dropdownFunc }
